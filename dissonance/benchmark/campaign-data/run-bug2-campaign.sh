...
# Task 69 M2 bug-2 campaign orchestrator (runs ON THE BOX, detached via setsid).
...
#
# Launch:  setsid nohup bash run-bug2-campaign.sh >~/t69m2-results/bug2-orch.out 2>&1 </dev/null &
# Watch:   tail -f ~/t69m2-results/bug2/progress.log     (and ~/t69m2-results/bug2/determinism.log)
...
run_campaign() {  # core name config seed
  ...
  taskset -c "$core" $BIN bench-campaign --bug 2 --config "$config" --seed "$seed" \
...
for seed in 1 2 3; do
  ...
  core=$(cat "$OUT/.csolo")
  run_campaign "$core" "b2-baseline-$seed-solo" baseline "$seed"
...
# --- Phase 3: compare co-tenant vs solo state_hash (P0 on divergence) -------
{
  ...
  for seed in 1 2 3; do
    co=$(grep "^b2-baseline-$seed " "$OUT/finds.log" | grep -o 'state_hash [0-9a-f]*' | head -1)
    so=$(grep "^b2-baseline-$seed-solo " "$OUT/finds.log" | grep -o 'state_hash [0-9a-f]*' | head -1)
    if [ -z "$co" ] && [ -z "$so" ]; then
