...
//!   `UPDATE_PUBLIC_API=1 cargo test -p revision-coordinator --test public_api`
//!
...
fn public_api_matches_snapshot() {
    ...
    assert_eq!(
        expected.trim_end(),
        actual.trim_end(),
        "public API of `{CRATE}` drifted from tests/public-api.txt. If this \
         change is intentional and reviewed, refresh the snapshot with:\n\
