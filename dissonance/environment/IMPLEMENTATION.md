...
- **CI wiring already in place.** `environment` is in the `public-api` job's `-p`
  list in `.github/workflows/quality.yml` (task 24); the `tests/public_api.rs`
  guard + the refreshed `tests/public-api.txt` snapshot pass on the pinned
  nightly (`nightly-2026-06-16`) and skip cleanly when the tooling is absent. No
  `miri` entry is needed (no `unsafe`). `Cargo.lock` is regenerated by the
  integrator — this branch does not commit it (matching the existing dissonance
  crates).
