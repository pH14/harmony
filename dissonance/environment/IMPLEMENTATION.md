...
- **CI wiring (one root-file edit left to the integrator, rule 1).**
  `environment` is already in the `public-api` job's `-p` list in
  `.github/workflows/quality.yml` (task 24); the `tests/public_api.rs` guard + the
  refreshed `tests/public-api.txt` snapshot pass on the pinned nightly
  (`nightly-2026-06-16`) and skip cleanly when the tooling is absent. **Add
  `-p environment` to the `kani` job** (next to `-p vtime -p lapic`) so the new
  `#[cfg(kani)]` harnesses run in CI — they pass locally under `cargo kani -p
  environment` (kani 0.67.0). No `miri` entry is needed (no `unsafe`). `Cargo.lock`
  is regenerated by the integrator — this branch does not commit it (matching the
  existing dissonance crates).

### Formal proofs (Kani)

`src/envcodec_proofs.rs` (`#[cfg(kani)]`, a child of `envcodec`) proves the bounded
integer invariants `compose` rests on, over fully symbolic `u64` inputs (strictly
stronger than the proptest sampling). All three verify (`cargo kani -p
environment` → 3 harnesses, 0 failures):
...
- `ratio_new_rejects_exactly_zero_denominator` — `Ratio::new` is total and returns
  ...
  `den() != 0` (the no-divide-by-zero invariant behind `SetClockRate`).
...
- `rekey_moment_is_injective` — distinct source `Moment`s that both re-key
  successfully map to distinct keys (the collision-free guarantee `saturating_add`
