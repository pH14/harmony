...
//! relies on, split out of `envcodec.rs` so they are `#[cfg(kani)]`-only: verified
//! by the dedicated `kani` CI job, never compiled into the normal/test build and
//! never seen by the mutation oracle. Declared as
//! `#[cfg(kani)] #[path = "envcodec_proofs.rs"] mod proofs;` in `envcodec.rs`, so
//! it is a child of `envcodec` and `use super::*` reaches the private
...
/// collapse two overrides onto one key (the round-1 P2 defect).
#[kani::proof]
...
/// that both re-key successfully map to distinct keys. This is the collision-free
/// guarantee that makes the override re-keying genesis-complete — exactly the
/// property `saturating_add` violated (it mapped every overflowing `Moment` to
/// `u64::MAX`).
#[kani::proof]
