...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rom = fs::read(PathBuf::from(env::var_os("HARMONY_SMB_ROM").ok_or("HARMONY_SMB_ROM")?))?;
    let core_path = PathBuf::from(env::var_os("HARMONY_QUICKNES_CORE").ok_or("HARMONY_QUICKNES_CORE")?);
    ...
    for step in schedule.split(',') {
        ...
        let chord = ButtonChord { buttons: u8::from_str_radix(b.trim_start_matches("0x"), 16)?, hold_frames: h.parse()? };
        ...
        println!("{frame:5}  {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {:04x} {dead}",
            w[0x770], w[0x772], w[0x0e], w[0xb5], w[0xce], w[0x75f], w[0x75c], w[0x746], w[0x71a], w[0x6d], w[0x86], w[0x1d]);
    }
    Ok(())
}
