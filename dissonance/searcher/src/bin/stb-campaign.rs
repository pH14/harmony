...
impl Args {
        ...
        I: IntoIterator<Item = OsString>,
    {
        ...
        if action_limit == 0 || action_limit > searcher::stb::archive::MAX_STB_ACTIONS {
            return Err(format!(
                "action-limit must be between 1 and {}",
                searcher::stb::archive::MAX_STB_ACTIONS
            )
            .into());
        }
        Ok(Self {
...
mod tests {
    ...
    fn fixed_execution_soak_is_explicit_and_valueless() {
        ...
        assert!(matches!(
            ...
            searcher::search::archive::RetentionPolicy::AdmitAlive
        ));
    }
