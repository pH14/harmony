...
//! Fault-library implementation of the game-neutral campaign interface.

use std::{collections::BTreeSet, error::Error, io::Write, path::PathBuf, sync::OnceLock};
...
/// Stream format written by fault-library campaigns.
pub const CAMPAIGN_STREAM_FORMAT: &str = "faultlab-consonance-campaign-stream-v1";
/// Snapshot checkpoint format written by fault-library campaigns.
...
/// Image identity shared by fault-library workers.
...
/// Fault-library campaign origin.
pub type FaultCampaignOrigin = CampaignOrigin<FaultGame>;
/// Fault-library resume checkpoint.
pub type FaultCampaignCheckpoint = CampaignCheckpoint<FaultlabSnapshot>;
/// Fault-library whole-tree snapshot checkpoint.
pub type FaultSnapshotCheckpoint = SnapshotCheckpoint<FaultlabSnapshot>;
/// Fault-library stream header.
pub type FaultCampaignStreamHeader = CampaignStreamHeader<FaultNoTableHeader>;
/// The generic campaign report of a fault-library run.
pub type FaultCampaignModeReport = CampaignModeReport<FaultAction, FaultArchiveReport>;
/// Fault-library progress sidecar record.
...
/// The campaign report plus the fault-library outcome: how many bugs the run
...
/// Fixed configuration for one live fault-library campaign.
...
impl FaultCampaignConfig {
    fn generic(&self) -> CampaignConfig<FaultGame> {
        CampaignConfig {
            ...
            // The fault-library campaign stops at its first bug and replays it.
...
fn recorded<'a>(policies: &'a GamePolicies, field: &str) -> Result<&'a str, Box<dyn Error>> {
    policies
        .get(field)
        .map(String::as_str)
        .ok_or_else(|| format!("fault-library stream is missing {field}").into())
}

fn merge_action_milestones(aggregate: &mut FaultMilestones, target: &FaultlabTarget) {
...
fn execute_job(
    target: &mut FaultlabTarget,
    origin_snapshot: &FaultlabSnapshot,
    replay: &[FaultAction],
...
impl Game for FaultGame {
    ...
    fn snapshot_memory_charge(snapshot: &FaultlabSnapshot) -> usize {
        ...
        if policies != &expected {
            for (field, value) in &expected {
                if recorded(policies, field)? != value {
                    return Err(
                        format!("fault-library stream {field} policy is not recognized").into(),
                    );
                }
            }
            return Err("fault-library stream carries an unknown game policy".into());
        }
        Ok(run)
    }
    ...
    fn reset(&self, target: &mut FaultlabTarget) {
        target.reset();
    }
    ...
    fn restore(
        &self,
        target: &mut FaultlabTarget,
        snapshot: &FaultlabSnapshot,
    ) -> Result<(), Box<dyn Error>> {
        target.restore(snapshot)
    }

    fn frames_clocked(&self, target: &FaultlabTarget) -> u64 {
    ...
    fn is_terminal(&self, target: &FaultlabTarget) -> bool {
    ...
    fn is_run_terminal(
        &self,
        _run: &FaultCampaignRun,
        target: &FaultlabTarget,
    ) -> Result<bool, Box<dyn Error>> {
        if target.exit_kind() != ExitKind::Ok && !target.found_bug() {
            return Err("the fault-library terminal predicate cannot inspect a failed VM".into());
        }
        Ok(target.found_bug())
    }

    fn snapshot(&self, target: &mut FaultlabTarget) -> Result<FaultlabSnapshot, Box<dyn Error>> {
        target
            .snapshot()
            .ok_or_else(|| "the fault-library endpoint has no successor to snapshot".into())
    }

    fn current_key(&self, target: &FaultlabTarget) -> Result<FaultArchiveKey, Box<dyn Error>> {
    ...
    ) -> Result<((), Option<FaultNoTableHeader>), Box<dyn Error>> {
        Ok(((), None))
    }
    ...
    fn expand_suffix_recorded(
        ...
        if before.is_some() {
            return Err("fault-library stream unexpectedly records a draw table".into());
        }
    ...
    fn retained_inputs_need_full(&self, _run: &FaultCampaignRun) -> bool {
        false
    }
    ...
    fn remember_draw_version(
        ...
        if required.is_empty() {
            ...
            Err("fault-library stream requires an unsupported draw-table version".into())
        }
    }
    ...
    ) -> &'a [crate::faultlab::archive::FaultArchiveEntryReport] {
        &source.entries
    }
    ...
    fn resume_input(&self, source: &FaultArchiveReport) -> Result<FaultInput, Box<dyn Error>> {
        source
            ...
            .ok_or_else(|| "the fault-library source archive has no retained entries".into())
    }
...
/// Run a fault-library campaign and return its report plus whole-tree
/// checkpoint.
///
/// # Errors
///
/// Returns an error when the campaign cannot run to completion.
pub fn run_faultlab_campaign_checkpointed(
    game: &FaultGame,
...
/// Replay a recorded fault-library stream exactly.
///
/// # Errors
///
/// Returns an error when the stream does not replay.
pub fn replay_faultlab_campaign_checkpointed(
    game: &FaultGame,
    stream_bytes: &[u8],
    origin_report: Option<&FaultArchiveReport>,
    origin_checkpoint: Option<&FaultCampaignCheckpoint>,
) -> Result<(FaultCampaignReport, FaultSnapshotCheckpoint), Box<dyn Error>> {
    let (report, checkpoint) =
