...
///
/// `#[serde(default)]` (container level) fills any missing field from
/// [`Default`], so a **partial** tuning file deserializes cleanly — `{}` is the
/// full default and `{ "fold_k": 128 }` overrides only that knob. This is the
/// "knobs must be config-visible" intent: an author tunes one dial without
/// restating the rest.
...
mod tests {
    ...
    fn partial_config_deserializes_with_defaults() {
        // `#[serde(default)]`: an empty object is the full default, and a partial
        // tuning file overrides only the named knob.
        let empty: CellConfig = serde_json::from_str("{}").expect("empty object");
        assert_eq!(empty, CellConfig::default());

        let partial: CellConfig =
            serde_json::from_str(r#"{"fold_k": 128}"#).expect("partial object");
        assert_eq!(
            partial,
            CellConfig {
                fold_k: 128,
        ...
        // Full config still round-trips exactly.
        let full = CellConfig::default();
        let json = serde_json::to_string(&full).expect("serialize");
        assert_eq!(
            serde_json::from_str::<CellConfig>(&json).expect("reparse"),
            full
        );
    }
}
