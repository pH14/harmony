...
/// Distinct assertions fingerprint distinctly (so dedup keeps distinct bugs).
#[test]
fn distinct_assertions_fingerprint_distinctly() {
    ...
    assert_ne!(base(1, vec![]), base(2, vec![]));
    assert_ne!(base(1, vec![1]), base(1, vec![2]));
