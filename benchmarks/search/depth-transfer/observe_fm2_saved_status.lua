...
for slot = 0, 5 do
    for _, field in ipairs({'offset', 'status', 'type', 'special', 'hp', 'x', 'y', 'nametable', 'saved_status'}) do
...
for frame = 1, frames do
    ...
    for offset = 0, 0x50, 0x10 do
        ...
        for _, value in ipairs({offset, status, read(0x6b02 + offset), special,
            read(0x40b + offset), read(0x401 + offset), read(0x400 + offset), read(0x6afb + offset), read(0x40c + offset)}) do
            slots[#slots + 1] = value
        end
    end
