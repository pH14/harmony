...
local frames = 120540
local trace = assert(io.open(out .. '/relevant-frames.csv', 'w'))
...
local function note_progress(frame, complete)
    local f = assert(io.open(out .. '/summary.json', 'w'))
...
assert(movie.load(input, true), 'movie failed to load')
assert(movie.active() and movie.readonly() and movie.ispoweron(), 'power-on read-only movie required')
assert(movie.length() == frames and movie.framecount() == 0, 'unexpected movie horizon or starting frame')
assert(string.lower(rom.gethash('md5')) == 'b2d2d9ed68b3e5e0d29053ea525bd37c', 'ROM payload checksum differs')
emu.speedmode('maximum')
for frame = 1, frames do
    assert(movie.mode() == 'playback', 'movie ended before its fixed horizon')
    emu.frameadvance()
    assert(movie.framecount() == frame, 'movie clock skipped or repeated a frame')
    ...
    if present ~= 0 or tagged or kraid ~= last_kraid or ridley ~= last_ridley then
        ...
        bytes = bytes + #line
        assert(bytes <= 32 * 1024 * 1024, 'trace exceeded output cap')
        trace:write(line)
        rows = rows + 1
    end
