...
| `blue-pilot.json` | Three exploratory seeds on Pokémon Blue to the Boulder Badge. The Game Boy workload has its own campaign binary and its own selector thresholds, so `blue-campaign` runs this panel rather than the common runner, which builds and drives the NES evaluation binary only. |
