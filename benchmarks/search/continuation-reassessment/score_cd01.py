#!/usr/bin/env python3
"""Reuse the frozen four-pair scorer for the distinct continuation Bombs comparison."""
import argparse
import hashlib
import json
...
def score(registration,evidence):
    ...
    result['format']='continuation-cd01-analysis-v1'
    result['decision']={'stop_persistence_line':'stop_continuation_bombs_line',
                        'earns_transfer_and_depth_decision':'earns_independent_confirmation'}.get(result['decision'],result['decision'])
    result['limits']='Prospective unchanged-continuation first-Bombs development, scored without historical or earlier panels. An allocation gate, not population significance or a boss/Wily result. Setup and unadmitted work remain additional unknowns.'
    return result
