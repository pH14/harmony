#!/usr/bin/env python3
"""One prospective, bounded ms02 qualification. Each cell runs at most once."""
...
def main(registration):
    ...
    if len(rows) == len(master['cells']) and all(r['valid'] for r in rows):
        try:
            activation = first_return(out / 'progress-control/campaign/stream.jsonl',
                                      out / 'progress-half/campaign/stream.jsonl')
        except (OSError, ValueError, KeyError, TypeError, AssertionError) as error:
    ...
    qualified = (len(rows) == len(master['cells']) and all(r['valid'] for r in rows)
                 and activation['activated'] and all(r['alternative_admissions'] > 0
                     for r in rows if r['cell'] != 'ordinary-control'))
    result = dict(format='scoped-return-sr01-result-v1', qualified=qualified, cells=rows,
