#!/usr/bin/env python3
"""Run the remaining five-witness compatibility block, stopping on the first failure."""
import argparse
...
def check_result(reg, case, folder):
    ...
    for mode in ('ordinary', 'one_frame'):
        ...
        # Metroid Evaluation::is_terminal uses death/victory/error, not in_play.
        # A complete held command can end outside mode3 after observing an item.
        assert 0 < state['health'] < 8000 and not state['ending']
...
def main():
    ...
    reg_path = args.protocol / 'fw02-registration.json'
    ...
    # These passes already completed in FW01; inspect them without replaying
    # or charging them again. Keep FW01's original failed checker unchanged.
    reused = reg['reused_fw01_case']
    assert check_result(reg, reused, args.protocol / reused['output'])['known_auxiliary_frames'] == 234966
    total = 0
    ...
    for case in reg['cases']:
        ...
        report = {'format': 'continuation-fw02-results-v1', 'registration_sha256': sha(reg_path),
