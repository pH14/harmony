...
def main():
    ...
    for index,(name,number) in enumerate(STAGES[:1] if a.qualification else STAGES):
        ...
        if result.returncode or not record.get('result',{}).get('solved'):
            manifest['status']='qualification_complete' if a.qualification and result.returncode==0 else 'stage_not_solved'
        ...
        if a.qualification:manifest['status']='qualification_complete'
            ...
            command=[str(a.progress_binary),'mm2',assets['core']['path'],assets['mm2']['path'],str(prefix),'wily4']
            with (a.out/'wily4-replay.json').open('w') as stream:
                subprocess.run(command,stdout=stream,stderr=subprocess.PIPE,check=True,timeout=120)
            replay=json.loads((a.out/'wily4-replay.json').read_text())
    ...
    manifest['cost_limitations']='Missing/incomplete stage results are incomplete accounting, never zero-cost successes. Prefix construction is separate from admitted frames; physical witness suffix costs appear per stage.'
