...
p.add_argument('root',type=pathlib.Path);p.add_argument('out',type=pathlib.Path);p.add_argument('target',type=pathlib.Path);p.add_argument('--features',default='');a=p.parse_args()
...
with (a.out/'build.log').open('wb') as log:
 subprocess.run(['cargo','build','--release','--locked','--manifest-path',str(a.root/'workloads/nes/Cargo.toml'),'--bin','nes-eval','--target-dir',str(a.target),'-j','7',*(['--features',a.features] if a.features else [])],env=env,stdout=log,stderr=subprocess.STDOUT,check=True)
...
metadata={'format':'harmony-search-build-v1',**before,'binary_sha256':evaluation.digest(a.out/'nes-eval'),'rustc':subprocess.check_output(['rustc','-Vv'],text=True),'cargo':subprocess.check_output(['cargo','-V'],text=True).strip(),'profile':'release','locked':True,'rustflags':env.get('RUSTFLAGS',''),'target_cache':str(a.target),'features':a.features}
