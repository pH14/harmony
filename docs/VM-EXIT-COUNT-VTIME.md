...
as a second host in the follow-up work of §5 when it goes idle; the plan's
milestones run entirely on the M1. The name says the clock's basis: the exit
ordinal — an observed count, like today's retired branches, over a coarser and
universally available event. In the vocabulary of
...
runs produce identical normalized logs and `state_hash` sequences — the
condition that catches a §2.4 trap left open, which every other check here
would miss (an unaudited binary reading the real counter diverges here and
nowhere else); no run hit the liveness watchdog; dmesg is free of RCU-stall
and soft-lockup reports; the inter-exit vns gap histogram is recorded and its
maximum stays under the tick period times a small documented factor;
throughput is reported next to the descriptive-mode x86 number for the same
payload.
