...
Apple silicon (2026-07-09), M4 nested-PMU result recorded (2026-07-11).** The hardware facts, three-mechanism analysis, rr evidence base,
...
The route has now been measured on Mac16,10 / M4 / macOS 26.5 and is **NO-GO for the current
retired-branch fast path**: the framework-emulated nested PMU exposes `PMCR.N=0` and no
programmable event counter. See the AS-2 disposition in `docs/APPLE-SILICON.md`.
...
for V-time — has now failed on the pinned M4 nested-PMU tuple and remains unproven on the
separate bare-metal/KVM candidates. The viability gate is substrate-specific hardware
measurement, not a code-cleanliness problem; the first completed Apple measurement returned
NO-GO. **Do not build an ARM backend behind an unpassed hardware gate; a clean trait boundary
cannot supply a missing PMU mechanism.**
...
The bet was narrower and harder than “does ARM virtualization work?”: whether Apple's nested
virtual PMU exposes a deterministic L2-only work event, then delivers overflow reliably with
bounded skid and supports exact ARM debug-step landing through L2 exceptions. On the pinned
M4 tuple the first clause is now answered NO: no programmable event counter exists, so the
overflow and landing clauses are blocked rather than untested by choice. A future framework
or hardware tuple would have to reopen AS-2 before the later questions become live again.
...
The pinned M4 tuple failed AS-2 because the named programmable counter mechanism is absent;
AS-3 and AS-4 therefore remain blocked unless a changed framework/hardware tuple reopens the
gate or a separately authorized counter contract replaces the current one.
...
Retain the Apple-silicon nested-EL2 program and its scoped AS-2 NO-GO in
`docs/APPLE-SILICON.md`; do not continue the pinned M4 fast path into overflow, Linux, or
snapshot work. Spark/Grace remain possible comparative KVM hosts with their own hardware
gates, not automatic replacements. Reopen Apple work only for a changed framework/hardware
tuple or a separately authorized counter/fallback contract. The measurement—not TCG
confidence or abstraction work—decides whether any hardware-accelerated ARM backend proceeds.
