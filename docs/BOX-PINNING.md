...
| Frontier determinism gates (task-58/65 box gates; Wave-5 gates 63/68/69/72; the 49/55/56 gate history) | leased core from {2, 1, 3} via `scripts/box-window.sh` | cpu10/9/11 | **Refined ruling (foreman, 2026-07-02): serialize the patched-KVM module *transitions*, parallelize the gates.** `box-window.sh acquire <name>` joins (or opens) the shared patched-module window and prints the leased core; `release <name>` — last lease out reverts to stock 1396736 + verifies. Up to 3 concurrent gates on distinct cores, siblings idle. **Measurement gates (skid, seal-rate — anything whose numbers are microarchitecture-sensitive) use `acquire <name> --exclusive`**: sole occupancy, no co-runners. Never load/rmmod KVM directly when using the coordinator. A lease lives for a TTL (default 30 min) whether or not the ssh shell that took it survives, so the natural `ssh <box> 'box-window.sh acquire <name>'` one-shot is safe (tasks/164 / hm-nvwx — liveness is time-or-pid, no longer pid alone); for a gate longer than the TTL pass `acquire … --ttl <seconds>` or `box-window.sh renew <name> [<seconds>]` before it lapses, and `release <name>` ends it early. A crashed gate self-heals when its TTL expires. |

Cores 1,3 are frontier-leasable via the window coordinator (above). When a new *standing* box workload appears, give it a core and record it here.
