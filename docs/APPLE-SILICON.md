...
Status: **AS-2 NO-GO on the pinned M4 tuple (2026-07-11).** This document records the
completed de-risking program and the bounded result; it is not a claim that every future
Apple-silicon or ARM substrate is infeasible. It narrows the ARM work in
...
**Observed disposition (2026-07-10): GO on the recorded M4; NO-GO remains scoped to the
recorded M1 Max.** Mac16,10 / Apple M4 on macOS 26.5 (25F71) passed 100 of 100 serialized
attempts with zero process or validator failures. Every 103,613-byte schema-v3 capability
manifest was byte-identical (SHA-256
`67142acb9818cc2eb65543566552992a38ea0374ddbc7dd924ee35c16e92f81e`), and the validator
classified the run set as `accepted`. The actual EL2-enabled VM/vCPU lifecycle and required
translation, GIC-backed timer, debug, PMU, and state surfaces all pass, so AS-1 may proceed on
this target. See the [M4 AS-0 acceptance analysis](../spikes/apple-silicon/results/as-0/mac16-10-apple-m4-25f71/m4-acceptance-20260710-v3/analysis.md).

The earlier MacBookPro18,2 / Apple M1 Max result is preserved: it returned
`el2_supported=false`, all independent EL2-enable requests returned `HV_UNSUPPORTED`, and
its ten byte-identical manifests form a coherent `valid_nonaccepting` observation. See the
[M1 AS-0 analysis](../spikes/apple-silicon/results/as-0/macbookpro18-2-apple-m1-max-25e253/el2-unavailable-confirmation-20260709-v1/analysis.md).
...
**Acceptance (operator-amended 2026-07-10):** at least 9,099 fresh-process runs yield
byte-identical ledgers and final architectural state for fixed payloads. EL2 and L2 execution
are distinguishable in the ledger. A malformed payload fails closed rather than hanging the
launcher. The original threshold was 10,000; evidence accepted under the amendment must retain
that history and bind a separate canonical operator-amendment record without rewriting the
original run-set.
...
The authoritative M4 campaign was operator-stopped on 2026-07-10. Its unmodified durable
ledger accounts for 9,099 attempted and completed fresh processes, zero failed attempts, one
unique 26,111-byte result SHA-256
`c14cf7268ffbee9118f3f12c0839db56ae7bdfb88522d4433bb52d27b25b30f2`, and empty stderr for
every attempt. Default validation still returns `accounting_error` for the original
failed-status 10,000-request run. Under the separate canonical operator amendment, the full
18,198-file corpus validates as `accepted_amended` with 9,099 observed against the exact 9,099
minimum. AS-1 is therefore GO on the pinned M4 tuple and AS-2 may proceed. See the
[operator-amended M4 analysis](../spikes/apple-silicon/results/as-1/mac16-10-apple-m4-25f71/m4-acceptance-amended-20260711-v2/analysis.md) and the immutable
[pre-amendment stop analysis](../spikes/apple-silicon/results/as-1/mac16-10-apple-m4-25f71/m4-operator-stopped-20260710-v1/analysis.md).
...
**Disposition (2026-07-11): NO-GO on Mac16,10 / Apple M4 / macOS 26.5 build 25F71.** The
authoritative 10-sample compact preflight executed 1,440 fresh VMs. Hypervisor.framework
emulated exactly PMUVer encodings 1 and 4 through 8, but every one of their 480 variants
reported `PMCR_EL0.N=0`. All 31 direct and indexed programmable slots on those emulated
surfaces were non-mutable; all unsupported rows produced the required exact UNDEF surface.
PMUVer 9 and `PMICNTR_EL0` were unavailable. Ten raw manifests differed only in live
`PMCCNTR_EL0` values and had one byte-identical normalized discovery projection. Every strict
validator report returned `valid_nonaccepting`, `finding=programmable_counter_absent`, no
errors, and only `PMCCNTR` as a contract-change alternative.

`PMCCNTR_EL0` counts processor cycles rather than the required frequency-independent retired
branch work and has not satisfied the required reset, L2-only filtering, overflow, or
determinism gates. Substituting it would be a new contract, not AS-2 acceptance. Because no
programmable event counter exists, the million-payload threshold is inapplicable rather than
waived: there is no candidate mechanism to measure. AS-3 through AS-9 remain blocked by the
named-hard-mechanism stop condition. See the [AS-2 disposition](../spikes/apple-silicon/results/as-2/mac16-10-apple-m4-25f71/m4-disposition-20260711-v1/analysis.md).
...
AS-2, AS-3, and AS-4 are the existential sequence. AS-2 has now returned the scoped NO-GO
above, so AS-3 and AS-4 are not permitted on the pinned tuple under this contract.
...
program. The AS-2 NO-GO means this implementation never reaches the AS-8/AS-9 prerequisite
for consolidation. Any future framework/hardware retest or alternate counter contract must
first reopen the failed gate; no consolidation work follows from this result.

## Immediate focus

AS-0 and AS-1 established a functioning nested test harness; AS-2 then established that the
pinned Hypervisor.framework tuple exposes no programmable nested event counter. The current
hardware-accelerated program is complete with a scoped NO-GO. Linux boot, AS-3 overflow,
snapshot work, and AS-8 integration are not permitted continuations of this goal. A future
retest on a changed framework/hardware tuple or a separately authorized TCG/single-step design
is a new program with a new contract.
