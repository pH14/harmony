...
| RDTSC | `vmx-exit(RDTSC-exiting)` | `EDX:EAX = VClock::tsc(work)` = 2 × V-ns (frozen 2.0 GHz, §2) | closes §7 "TSC plumbing"; host TSC never reaches the guest — docs/PLAN.md trap table (RDTSC → f(V-time)). **Backend-dependent:** RDTSC-exiting is not surfaced by stock KVM (which would give host-time TSC-offset, not V-time) — requires the [question]-Backend VMX backend (§1). | SDM Vol.3C §25.6.2 (RDTSC exiting); docs/PLAN.md trap table; §1/[question] Backend; consonance/vtime/src/clock.rs (VClock::tsc); §2 (frozen TSC) |
