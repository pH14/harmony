...
`consonance/harmony-linux/linux/config-fragment`). The model is a config artifact: its canonical serialization
...
| 0x1 | 0 | EAX | const(0x00050654) | Frozen family/model/stepping = 06_55H stepping 4 (Skylake-SP): a plain, well-trodden server identity (rr documents per-µarch PMU quirks; modern Intel server is the best-supported path per docs/PLAN.md risks); model 0x55 keeps Linux's SKX-specific quirk paths consistent with the 25 MHz crystal | SDM Vol.2 CPUID leaf 01H; docs/PLAN.md "Known risks"; arch/x86/kernel/tsc.c:702–711 |
