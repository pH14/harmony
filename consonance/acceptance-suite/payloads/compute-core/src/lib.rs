...
//! in `consonance/acceptance-suite/golden/compute.txt` is derived from the same code that runs in
...
mod tests {
    ...
    fn digest_matches_golden() {
        ...
        .expect("consonance/acceptance-suite/golden/compute.txt must exist");
        let line = golden
            .lines()
