...
- **10 payloads**: `insn-{rdtsc,rng,cpuid,rdpmc,hlt,mwait}`,
  ...
  `payloads/run-tests.sh` and `consonance/acceptance-suite/golden/`.
...
> `consonance/acceptance-suite/golden/<name>.digest`, gate run by `consonance/vmm-core/tests/box_corpus.rs`).
...
| Gate | Status |
|------|--------|
| Part-A: build, run **twice** byte-identical under QEMU, goldens committed, `make -C consonance/acceptance-suite test-payloads` green | ✅ (15 payloads, macOS QEMU 11.0.1) |
| 1 — trap-surface coverage checklist | ✅ `README.md` |
| 2 — manifest registration (`micro` + oracles) | ✅ `acceptance-suite validate --manifest docs/corpus-manifest.toml` passes (10 items, round-trips) |
...
| 4 — **box** determinism (O1) + O2 conformance goldens + `irq-landing` sweep | 🔧 **built by task 28** (report channel `0x0CA2` + VMM-backed `acceptance-suite` Machine + O2 goldens + `consonance/vmm-core/tests/box_corpus.rs`); the live box run/goldens are blessed on the box at review |

## What the integrator must know

1. **Manifest loads under the merged harness.** After #48 merged, the manifest was
   rewritten to acceptance-suite's string-token form (`kind = "micro"`,
   `oracles = ["determinism", "seed_sensitivity:pure"|":rng"]`);
   `cargo run -p acceptance-suite -- validate --manifest docs/corpus-manifest.toml`
...
2. **O2/Conformance — built by task 28 (corpus box-integration).** What task 18
   ...
   `0x0CA2` channel (not `0x0CA1`, owned by #44); a VMM-backed `acceptance-suite`
   `Machine` (`vmm_core::corpus`) loads each payload through the
   `PatchedKvmBackend`, runs `check_determinism` (O1) + captures the
   `observable_digest` (O2). `"conformance"` + a `golden = consonance/acceptance-suite/golden/<name>.digest`
   ...
   **distinct** from the run-tests.sh **serial-shape** goldens
   (`consonance/acceptance-suite/golden/<name>.txt`).
...
- These are bare-metal payloads: not built/tested under Miri (the target is
  ...
  `consonance/acceptance-suite/payloads/` and the Part-A gate.
