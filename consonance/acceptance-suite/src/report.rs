    ...
    G: Fn(&str) -> Option<String>,
{
    ...
    for &oracle in &item.oracles {
        let result = match oracle {
            ...
            OracleKind::Conformance => match item.golden.as_deref().and_then(&read_golden) {
            ...
            OracleKind::SeedSensitivity { rng_consuming } => {
...
mod tests {
    ...
    fn run_item_runs_each_declared_oracle_in_order() {
        ...
        assert_eq!(report.results[0].oracle, OracleKind::Determinism);
        assert_eq!(report.results[1].oracle, OracleKind::Conformance);
        assert_eq!(
