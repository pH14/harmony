...
impl OracleKind {
    ...
    pub(crate) fn from_token(s: &str) -> Option<OracleKind> {
        match s {
            "determinism" => Some(OracleKind::Determinism),
            "conformance" => Some(OracleKind::Conformance),
            "seed_sensitivity:rng" => Some(OracleKind::SeedSensitivity {
                rng_consuming: true,
            }),
            "seed_sensitivity:pure" => Some(OracleKind::SeedSensitivity {
                rng_consuming: false,
            }),
            _ => None,
        }
...
pub fn check_determinism<F: SubjectFactory>(
    ...
    let oracle = OracleKind::Determinism;
...
pub fn check_seed_sensitivity<F: SubjectFactory>(
    ...
    let oracle = OracleKind::SeedSensitivity { rng_consuming };
    if seed_a == seed_b {
        return Ok(OracleResult {
            oracle,
            passed: false,
...
mod tests {
    ...
    fn oracle_token_round_trip() {
        for o in [
            ...
            assert_eq!(OracleKind::from_token(o.to_token()), Some(o));
        }
        assert_eq!(OracleKind::from_token("nonsense"), None);
    }
}
