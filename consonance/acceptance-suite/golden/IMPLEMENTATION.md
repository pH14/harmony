...
Refresh of the stale `consonance/acceptance-suite/golden/insn-cpuid.digest` O2 conformance golden.
...
- `consonance/acceptance-suite/golden/insn-cpuid.digest`: `746d8bbb…16a9` → `cd321ad6…82f5`
  (one line; the only content change).
- `consonance/acceptance-suite/golden/insn-cpuid.provenance.md`: new adjacent provenance doc.
...
test on macOS); acceptance-suite manifest tests use placeholder golden paths and only
require conformance items to *have* a golden. So the portable checks are:
acceptance-suite builds/tests green, manifest still validates, and the golden is a
...
- Refresh recipe (for the next time the frozen CPUID/MSR model changes): on the
  ...
  consonance/acceptance-suite/golden/*.digest`, update `insn-cpuid.provenance.md`, commit. Never
  hand-edit a `.digest`.
