...
//! Gate 5 — the `det-corpus` binary: real `std::process::Command` runs over toy
...
/// Found via the shared `det_corpus::toy_factory`, so it cannot drift from what
...
fn find_source(pred: impl Fn(bool, bool) -> bool, what: &str) -> String {
    for n in 0u64..2000 {
        let s = n.to_string();
        let f = det_corpus::toy_factory(&s);
...
source = \"1\"
oracles = [\"determinism\", \"conformance\"]
golden = \"guest/golden/alpha.digest\"
",
    );
