# det-corpus — implementation notes
...
- `src/oracle.rs` — `Oracle`, `OracleResult`, and `check_determinism` /
...
- `src/main.rs` — the `det-corpus run` / `det-corpus validate` binary with the
...
`consonance/det-corpus/`. `observable_digest()` hashes only the guest's
...
3. **O3 default seeds collided after normalization.** Default `--seed 0` derived a
   ...
   (`det_corpus::toy_factory`) so the binary and tests build identical factories.
...
- **Serde-encoding the `Oracle` enum directly in TOML** — its mixed unit/struct
...
- **Toy registry only.** `det_corpus::toy_factory(source)` maps an item's
...
- **Public-API snapshot** (`tests/public-api.txt`) is frozen; refresh with
  `UPDATE_PUBLIC_API=1 cargo test -p det-corpus --test public_api -- --ignored`
