...
pub struct CorpusItem {
    ...
    /// Path to the golden digest. Required iff [`Oracle::Conformance`] is in
...
impl ItemDto {
    fn into_item(self) -> Result<CorpusItem, ManifestError> {
        ...
        for tok in &self.oracles {
            let o = Oracle::from_token(tok).ok_or_else(|| {
                ManifestError::new(format!(
...
/// and every [`Oracle::Conformance`] item must carry a `golden`. On failure the
...
pub fn validate(items: &[CorpusItem]) -> Result<(), ManifestError> {
    ...
    let missing: Vec<&str> = items
        .iter()
        .filter(|it| {
            it.oracles.iter().any(|o| matches!(o, Oracle::Conformance)) && it.golden.is_none()
        })
        .map(|it| it.name.as_str())
        .collect();
    if missing.is_empty() {
...
mod tests {
    ...
    fn item(
        ...
        CorpusItem {
            name: name.to_string(),
            kind,
            source: format!("guest/payloads/{name}.bin"),
            oracles,
    ...
    fn round_trip_directed() {
        let items = vec![
            item(
                "tsc",
                CorpusKind::Micro,
                vec![Oracle::Determinism, Oracle::Conformance],
                Some("guest/golden/tsc.digest"),
            ),
            item(
                "rdrand",
                CorpusKind::Micro,
