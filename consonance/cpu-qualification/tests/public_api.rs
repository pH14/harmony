...
//!   `UPDATE_PUBLIC_API=1 cargo test -p cpu-qualification --test public_api`
//!
//! Requires the pinned nightly toolchain, `cargo-public-api`
//! (`scripts/install-quality-tools.sh`), and the Linux target's standard
//! library. When any is absent the test skips loudly rather than failing, so a
//! plain `cargo nextest` on a stable-only box stays green; CI installs them, so
//! the gate runs for real there.
...
const CRATE: &str = "cpu-qualification";

/// The measurement modules are `cfg(target_os = "linux")`, so the surface a
/// macOS host sees and the surface the Linux CI runner sees are different
/// crates' worth of items. Freezing one named target makes the snapshot a
/// property of the source rather than of whoever ran the test.
const SNAPSHOT_TARGET: &str = "x86_64-unknown-linux-gnu";
...
fn public_api_matches_snapshot() {
    ...
    if !output.status.success() {
        ...
        // Missing tool / toolchain / target -> skip; a real build error -> fail.
    ...
    assert_eq!(
        expected.trim_end(),
        actual.trim_end(),
        "public API of `{CRATE}` drifted from tests/public-api.txt. If this change is \
         intentional and reviewed, refresh the snapshot with:\n  \
