...
//!
//! Regenerates this crate's public surface with `cargo public-api` on the pinned
//! nightly toolchain and asserts it byte-matches the committed
//! `tests/public-api.txt`. Any drift in the frozen public contract becomes a
//! failing test and a reviewable diff. `pit`'s public surface (the `PitState`
//! snapshot seam, plus the port/frequency constants `vmm-core` addresses the device
//! by) is a contract.
//!
//! Refresh after an intentional, reviewed API change:
//!   `UPDATE_PUBLIC_API=1 cargo test -p pit --test public_api`
//!
...
/// Pinned nightly — `cargo-public-api` needs rustdoc-JSON, which is nightly-only.
/// Keep in sync with `docs/CODE-QUALITY.md` and the `public-api` CI job.
...
fn public_api_matches_snapshot() {
    ...
    let output = match Command::new("cargo")
        .args([
            ...
            "-p",
            CRATE,
            // Freeze the full surface (all features), so a public item gated behind
            // a non-default feature can never drift unnoticed.
            "--all-features",
            "-sss",
            "--color",
            "never",
