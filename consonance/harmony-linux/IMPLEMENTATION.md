...
protocol do not change. Its raw-frame ioctl is the only x86 agent transport:
agents no longer map the pages through `/dev/mem` or obtain I/O privilege. A
global mutex therefore covers every application and agent exchange from request
staging through the synchronous `OUT` and response copy. The sequence counter
plus per-open pending entropy bytes live in guest memory, so snapshot/replay
captures all future-affecting state.
...
On 2026-07-20, `make -C consonance/harmony-linux test-linux` passed on the determinism box
