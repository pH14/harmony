...
# Produces consonance/harmony-linux/build/initramfs-maze.cpio.gz.
#
...
# FLOW_AGENT_BIN pattern); by default build it from consonance/harmony-linux/maze-agent/.
if [ -n "${MAZE_AGENT_BIN:-}" ]; then
    echo "== maze image: using prebuilt maze-agent: $MAZE_AGENT_BIN"
    AGENT_BIN=$MAZE_AGENT_BIN
else
    echo "== maze image: building maze-agent (consonance/harmony-linux/maze-agent)"
