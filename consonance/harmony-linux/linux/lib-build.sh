...
# consonance/harmony-linux/linux/ with CWD = consonance/harmony-linux/linux/. Every entry point selects
...
verify_and_extract() {
    ...
    dir=$3
    if [ ! -f "$tarball" ]; then
        echo "FAIL: $tarball missing — run 'make -C consonance/harmony-linux fetch' first (needs network once)" >&2
        exit 1
    fi
