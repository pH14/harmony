...
# FLOW_AGENT_BIN pattern); by default build it from consonance/harmony-linux/play-agent/.
if [ -n "${PLAY_AGENT_BIN:-}" ]; then
    echo "== game image: using prebuilt play-agent: $PLAY_AGENT_BIN"
    AGENT_BIN=$PLAY_AGENT_BIN
else
    echo "== game image: building play-agent (consonance/harmony-linux/play-agent)"
