...
//!
//! The frames here are built by hand against the host's documented layout
//! rather than through the host encoder, so a silent change on either side of
//! the contract fails this test.

use environment::{DecisionClass, Fault, Span, process_target};
use harmony_fault_agent::faults::ActiveFaults;
use harmony_fault_agent::supervisor::{Action, Supervisor};
use hypercall_proto::parse_standing;
