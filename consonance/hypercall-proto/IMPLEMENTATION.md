...
- Service id `8` is the ordered `Payload` source. `Client::payload_fetch` sends one
  four-byte expected length, requires an exact-size response, rejects empty and
  over-frame destinations locally, and surfaces host exhaustion as
  `ClientError::Status(Status::OutOfRange)`. The protocol goldens pin id 8 and prove
  one successful consume followed by loud exhaustion.
