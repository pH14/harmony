...
//! Run CPU-pinned per `docs/HARDWARE-TESTING.md`, reverting KVM to stock afterwards:
...
use fault_policy::{EnvSpec, FaultPolicy};
...
fn server(seed: u64) -> DynServer {
        ...
        Box::new(move || Ok(boot_demo(&payload, seed)));
    {
        ...
        server.set_service_factory(fault_policy::consonance::service_factory());
        server
    }
}
...
fn branch_env(seed: u64, buggify_fires: bool) -> Reproducer {
    ...
    Reproducer {
        ...
        bytes: fault_policy::consonance::translate(&spec).unwrap().encode(),
    }
}
