...
//! (`KVM_CAP_X86_DETERMINISTIC_INTERCEPTS`), and the `x86-kvm`
...
//! cd ../..
//! taskset -c 2 cargo test -p acceptance-tests --test box_corpus -- --ignored --nocapture
//! ```
//!
...
//!
//! ```sh
//! DETCORPUS_BLESS=1 taskset -c 2 cargo test -p acceptance-tests --test box_corpus \
//!     -- --ignored --nocapture
//! ```
//!
...
/// Repo root, from this crate's manifest dir (`consonance/acceptance-tests`).
        ...
        boot_patched_corpus(&self.payload, GUEST_RAM_LEN, seed).unwrap_or_else(|e| {
            panic!(
                "boot_patched_corpus({}) failed: {e}. Needs the LOADED patched KVM \
                 (KVM_CAP_X86_DETERMINISTIC_INTERCEPTS), perf_event, and the x86-kvm host. \
...
#[test]
#[ignore = "box-only: needs the LOADED patched KVM + perf + x86-kvm host + built C1 payloads; \
...
fn c1_corpus_o1_diagnostic() {
    ...
    for item in &items {
        ...
        let (sa, sb) = (a.vmm().state_hash().unwrap(), b.vmm().state_hash().unwrap());
...
fn c1_corpus_o1_repeat_diagnostic() {
    ...
    for name in targets {
        ...
        for i in 0..N {
            ...
            let (mha, mhb) = (a.state_hash().unwrap(), b.state_hash().unwrap());
            let (sva, svb) = (a.vmm().state_hash().unwrap(), b.vmm().state_hash().unwrap());
