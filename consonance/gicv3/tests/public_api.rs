...
//! failing test and a reviewable diff. `gicv3`'s public surface (the
//! `GicState` snapshot seam the arm64 vendor's device blob consumes, plus the
//! frame/timer constants `vmm-core` addresses the model by) is a contract.
//!
//! Refresh after an intentional, reviewed API change:
//!   `UPDATE_PUBLIC_API=1 cargo test -p gicv3 --test public_api`
//!
...
/// nightly-only. Keep in sync with `CONTRIBUTING.md` and the `public-api`
/// CI job.
const PINNED_NIGHTLY: &str = "nightly-2026-06-16";
const CRATE: &str = "gicv3";
