...
//! `PmuBranchCounter` — the **box-only syscall orchestration** for the backend's
//! retired-conditional-branch counter that drives `Backend::run_until`'s
//! overflow-early phase (`#[cfg(target_os = "linux")]`).
//!
//! It opens the `perf_event` counter with the pure [`crate::pmu`] config
//! ([`branch_counter_attr`]) and adds the box-only machinery: a ring-buffer `mmap`,
//! async overflow notification (`O_ASYNC` + `F_SETOWN_EX` to the vCPU TID + a no-op
//! `SIGIO` handler installed without `SA_RESTART`) so an overflow **`EINTR`s the
//! in-flight `KVM_RUN`** on this thread — the host-side "kick" that breaks a
//! busy-spinning guest out at the armed branch count (not a guest PMI;
//! signal-delivery latency is part of the skid the `skid_margin` bounds).
//!
//! Like [`crate::kvm_sys`] / vmm-core's `work_perf`, this is box-only FFI: every
//! function issues a `perf_event`/`fcntl`/`mmap` syscall (or a trivial wrapper)
//! that cannot run without bare-metal Intel + perf, so the coverage + mutation
//! oracles (which run with no perf) cannot reach it — it is **excluded** from those
//! gates (`.cargo/mutants.toml`, the coverage `--ignore-filename-regex`) and
//! verified instead by the `#[ignore]` box tests + Miri (behind `#[cfg(not(miri))]`
//! seams). The *config* it opens is the pure, gate-covered [`crate::pmu`].
//!
//! ## Box-validation notes
//!
//! The overflow mechanism — that `PERF_EVENT_IOC_PERIOD` re-arms the next overflow
//! immediately (true on the box's 5.17+ kernel), that the `SIGIO` reliably
//! `EINTR`s `KVM_RUN`, and that the ring buffer is drained so a long run never
...
/// The signal an overflow delivers. `O_ASYNC` with the default `F_SETSIG` generates
/// `SIGIO`; a no-op handler (no `SA_RESTART`) turns it into a `KVM_RUN` `EINTR` kick.
/// (Repurposing `SIGIO` is safe in the single-vCPU VMM — no other async I/O here.)
...
impl PmuBranchCounter {
    ...
    /// calling thread (CPU-pinned, `docs/BOX-PINNING.md`), wired for async overflow
    /// signals, using the pure [`branch_counter_attr`] config.
    ///
    /// # Errors
    /// [`BackendError::Io`] if `perf_event_open` / `mmap` / `fcntl` / the enable
    /// ioctl fails. Opening is **non-fatal to the rest of the backend**: the caller
    /// stores the failure and only `run_until` surfaces it.
    pub(crate) fn open() -> Result<PmuBranchCounter> {
        install_overflow_handler();
        let attr = branch_counter_attr();
