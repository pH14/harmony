...
>   round 4 re-run, commit `ab1b749`: sealed at the first task-39-rejected point, step 154221;
>   `task39_rejected=3112`, `genuine_inflight=1` (the round-4 `genuine_inflight >= 1` gate assertion holds —
>   the V-time-deterministic workload reaches exactly one LAPIC-IRR-pending point every run); gate 1/2/3
>   all ✓, full-hash `66b4d4b4…85884164`, KVM reverted to stock `1396736`.)
