...
>   round 7 re-run, commit `bfad3cb`: sealed at the first task-39-rejected point, step 154221;
...
>   `KVM_SET_VCPU_EVENTS → EINVAL`; this confirmed which validity bits the box's KVM accepts). The **round-7
>   `triple_fault`/`payload` save-reject** (§4) does **not** reject the residual seal points (they carry
>   neither field), so gate 1/2/3 are unchanged — all ✓, full-hash `66b4d4b4…85884164` **unchanged**
