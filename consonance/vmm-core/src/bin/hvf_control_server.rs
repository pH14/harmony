...
fn main() -> std::process::ExitCode {
    ...
    for session in 0..max_sessions {
        ...
        let Some(trace) = vmm.virtual_time_trace() else {
            eprintln!("HVF control session {session} ended without a virtual_time trace");
            return std::process::ExitCode::FAILURE;
        };
        println!(
        ...
        if let Some(directory) = std::env::var_os("HARMONY_CONTROL_DUMP_DIR") {
            ...
            let session_trace_path =
                directory.join(format!("hvf-session-{session}.virtual_time.log"));
            ...
            println!(
                "HVF_CONTROL_DUMP session={session} ram={} vcpu={} virtual_time_log={}",
                ram_path.display(),
                vcpu_path.display(),
                session_trace_path.display(),
            );
