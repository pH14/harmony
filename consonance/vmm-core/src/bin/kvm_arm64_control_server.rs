...
fn main() -> std::process::ExitCode {
    ...
    let (Some(image_path), Some(initramfs_path), Some(socket_path)) =
        ...
        eprintln!(
            "usage: kvm_arm64_control_server <Image-game> <initramfs-game.cpio.gz> \
             <socket> [max-sessions]"
        );
    ...
    println!(
        "KVM_CONTROL_SERVER_READY socket={} max_sessions={max_sessions}",
    ...
    for session in 0..max_sessions {
        ...
        if let Some(path) = std::env::var_os("HARMONY_PORTABLE_IMPORT") {
            ...
            let file = match std::fs::File::open(&path) {
                ...
                Err(error) => {
                    eprintln!(
                        "KVM session {session} cannot open {}: {error}",
                        path.display()
                    );
                    return std::process::ExitCode::FAILURE;
                }
            ...
            match server.import_portable_snapshot(std::io::BufReader::new(file)) {
                Ok(receipt) => println!(
                    "KVM_PORTABLE_IMPORT session={session} id={} at={} sdk_events={} trace_events={} trace_schedules={} tainted={} state_hash={} path={}",
                    receipt.id.0,
                    receipt.at.0,
                    receipt.sdk_events,
                ...
                Err(error) => {
                    eprintln!("KVM session {session} portable import failed: {error}");
                    return std::process::ExitCode::FAILURE;
                }
            }
        }
            ...
            (Some(path), Some(handle)) => {
                ...
                let handle = if handle == "last" {
                    match server.latest_snapshot() {
                        Some(handle) => handle,
                        None => {
                            eprintln!("KVM session {session} has no snapshot to export");
                            return std::process::ExitCode::FAILURE;
                        }
                    }
                } else {
                ...
                let file = match std::fs::File::create(&path) {
                    ...
                    Err(error) => {
                        eprintln!(
                            "KVM session {session} cannot create {}: {error}",
                            path.display()
                        );
                        return std::process::ExitCode::FAILURE;
                    }
                ...
                match server.export_portable_snapshot(handle, std::io::BufWriter::new(file)) {
                    Ok(receipt) => println!(
                        "KVM_PORTABLE_EXPORT session={session} id={} at={} sdk_events={} trace_events={} trace_schedules={} tainted={} state_hash={} path={}",
                        receipt.id.0,
                        receipt.at.0,
                        receipt.sdk_events,
                        receipt.trace_events,
                    ...
                    Err(error) => {
                        eprintln!("KVM session {session} portable export failed: {error}");
                        return std::process::ExitCode::FAILURE;
                    }
                }
            }
        ...
        {
            eprintln!("KVM control session {session} placement check failed: {error}");
            return std::process::ExitCode::FAILURE;
        }
        println!(
            "KVM_CONTROL_SESSION_TRACE session={session} segments={} portable_events={} \
        ...
        if let Some(path) = std::env::var_os("HARMONY_ARM64_ARCH_DUMP") {
            ...
            let state = match vmm.arm64_architectural_state() {
                Ok(state) => state,
                Err(error) => {
                    eprintln!("KVM session {session} architectural capture failed: {error}");
                    return std::process::ExitCode::FAILURE;
                }
            };
            if let Err(error) = std::fs::write(&path, format!("{state:#?}\n")) {
                eprintln!(
                    "KVM session {session} cannot write {}: {error}",
                    path.display()
                );
                return std::process::ExitCode::FAILURE;
            }
            println!(
                "KVM_ARM64_ARCH_DUMP session={session} path={}",
                path.display()
            );
        }
        if let Some(directory) = std::env::var_os("HARMONY_CONTROL_DUMP_DIR") {
            ...
            let trace_file = match std::fs::File::create(&session_trace_path) {
                ...
                Err(error) => {
                    eprintln!(
                        "KVM control session {session} could not create {}: {error}",
...
fn main() -> std::process::ExitCode {
    eprintln!("kvm_arm64_control_server requires a Linux/aarch64 KVM host outside Miri");
