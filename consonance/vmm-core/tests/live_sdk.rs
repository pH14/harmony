...
//!
//! Box-only: needs the LOADED patched `/dev/kvm`; `#[ignore]`d so a plain `cargo nextest` shows it not-run.
...
fn boot_demo(payload: &[u8], seed: u64) -> vmm_core::vmm::Vmm<Box<dyn Backend<A = X86>>> {
    let mut vmm = boot_selected(BackendKind::Patched, payload, GUEST_RAM_LEN, seed)
        .expect("boot_selected(Patched, sdk-demo) — needs the LOADED patched KVM + perf");
...
/// `state_hash`.
#[test]
#[ignore = "box-only: needs the LOADED patched KVM + perf"]
