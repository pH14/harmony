...
//! artifacts, and the `x86-kvm` host — none of which exist in the default
...
#[test]
#[ignore = "box-only live gate (real KVM + built guest image + x86-kvm host); run on \
...
/// milestone. Box-only (real KVM + built guest image + x86-kvm host).
#[test]
...
/// task 34 / `README.md`.
#[test]
#[ignore = "box-only determinism gate (LOADED patched KVM + built guest image + x86-kvm host); \
