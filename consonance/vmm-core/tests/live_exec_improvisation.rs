...
fn boot_pg(kernel: &[u8], initramfs: &[u8], seed: u64) -> DynVmm {
    ...
    .expect("boot_linux_selected (patched) — needs the LOADED patched KVM + perf + x86-kvm host")
}
...
#[test]
#[ignore = "box-only improvisation gate (LOADED patched KVM + built image + x86-kvm host); \
