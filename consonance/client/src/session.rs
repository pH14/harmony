...
#[cfg(target_arch = "x86_64")]
fn controlled_image_identity_with_config_and_payloads(
    kernel: &[u8],
    initramfs: &[u8],
    ...
    let mut digest = Sha256::new();
    digest.update(b"consonance-controlled-session-image-v1\0");
    update_digest_field(&mut digest, kernel);
    update_digest_field(&mut digest, initramfs);
    digest.update((config.ram_bytes as u64).to_le_bytes());
    digest.update(config.seed.to_le_bytes());
    digest.update(config.run_budget.to_le_bytes());
    update_digest_field(&mut digest, config.cmdline.as_bytes());
    digest.update(profile);
    digest.update((setup_payloads.len() as u64).to_le_bytes());
...
mod tests {
    ...
    #[cfg(target_arch = "x86_64")]
    #[test]
    fn controlled_image_identity_binds_profile_launch_and_setup_inputs() {
        let config = SessionConfig::new(128 * 1024 * 1024, 7, 11, "cmdline");
        let payloads = vec![vec![0, 1], vec![2, 3]];
        let profile = [0xA5; 32];
        let identity = controlled_image_identity_with_config_and_payloads(
            b"kernel",
            b"initramfs",
            &config,
            &payloads,
        ...
        assert_ne!(
            identity,
            controlled_image_identity_with_config_and_payloads(
                b"other",
                b"initramfs",
                &config,
                &payloads,
        ...
        assert_ne!(
            identity,
            controlled_image_identity_with_config_and_payloads(
                b"kernel", b"other", &config, &payloads, profile,
            )
        );
        ...
        changed = config.clone();
        changed.cmdline.push_str(" changed");
        assert_ne!(
            identity,
            controlled_image_identity_with_config_and_payloads(
                b"kernel",
        ...
        let mut scheduling = config.clone();
        scheduling.wall_limit = Some(Duration::from_secs(1));
        scheduling.defer_virtual_time_checkpoint_hashes = true;
        assert_eq!(
            identity,
            controlled_image_identity_with_config_and_payloads(
                b"kernel",
