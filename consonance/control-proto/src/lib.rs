...
/// `hello` rather than disagree only when a larger read is attempted. Version
/// 11 adds the standing-fault poll service and the [`CapFlags::STANDING_POLL`]
/// advertisement, so a peer knows a `Branch` may carry standing faults.
