...
- `src/main.rs` — the one-shot Linux binary: self-check witness → `net_decide` over
  the `vmcall-transport` doorbell → install enforcement. The privileged doorbell
...
- Own workspace (like `guest/sdk`), so the root `cargo deny`/`clippy` do not fold
  it in; run its gates from `guest/flow-agent/`.
