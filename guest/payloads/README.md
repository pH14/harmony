# guest/payloads/ — bare-metal Part-A payloads

Tiny Multiboot-v1 payloads with fully known, timing-independent serial output,
checked byte-for-byte against `guest/golden/`. The boot/UART/exit/IDT pipeline
...
(`guest/golden/<name>.digest`, box-captured) for the six payloads that reach a
