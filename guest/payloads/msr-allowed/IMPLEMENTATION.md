...
only `guest/payloads/msr-allowed`, `guest/payloads/contract-data`, the README
...
- **`msr-allowed` payload**: the round-trip loop now iterates
  ...
  `guest/golden/msr-allowed.txt` is unchanged.
...
- **Gate 3 (O2 golden)** — the report stream expanded, so the box-captured
  `guest/golden/msr-allowed.digest` is now **unblessed** (placeholder content, the
...
```sh
cd guest/payloads
git diff --relative=guest/payloads <base>...HEAD -- guest/payloads/contract-data/src/lib.rs > /tmp/cd.diff
...
**root** workspace, which `exclude`s `guest/`, so this was run manually.
...
```sh
cd guest/payloads && cargo build --release            # build the expanded payload
cd ../..
...
git diff guest/golden/msr-allowed.digest
