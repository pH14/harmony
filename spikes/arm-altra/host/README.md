# `host/` — the kvm/arm64 determinism patch draft (0004-analogue)

**UNTESTED ON SILICON. DRAFT.** This directory holds the arm64 analogue of the
x86 "0004" determinism patch and the machinery to prove it *applies and compiles*
against a pinned kernel tree. It has **never produced a booted host kernel**, made
...
any gate, and the final check disassembles the built `arm.o`/`handle_exit.o` and
asserts the patch's constants (the `0xe4` ioctl, the cap number, the flag bit in
*both* the arm and the fire sites, and the `KVM_EXIT_PREEMPT` literal) are present
in the machine code — a source grep would pass on a patch that compiled to nothing.
