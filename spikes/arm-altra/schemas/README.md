...
| `fixtures/` | Nineteen checked-in run-sets — two the checker must accept (a patched AA-3 landing run and an AA-1 counting run), seventeen it must reject (one per failure mode). Generated from the oracle model by `gen-fixtures`; committed as files because they are the evidence the tests read. |
