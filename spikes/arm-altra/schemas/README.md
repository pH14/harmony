...
| `fixtures/` | Twenty-two checked-in run-sets — **four** the checker must accept (a patched AA-3 landing run, an AA-1 counting run, an AA-1(c) early/late skid-distribution run, and an AA-6 same-input gate), **eighteen** it must reject (one per failure mode). Generated from the oracle model by `gen-fixtures`; committed as files because they are the evidence the tests read. |
