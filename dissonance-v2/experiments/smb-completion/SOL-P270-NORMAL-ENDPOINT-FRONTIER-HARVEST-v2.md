...
# Sol p270 normal-endpoint frontier harvest v2
...
Continue the existing normal SMB endpoint search from the sole registered p270
adoption. This is one adoptable harvest, not a policy comparison. The immutable
p270 endpoint is the comparator and every live draw is ordinary endpoint search.

The binary may read only the exact adopted `SmbInput` named below and the ROM.
It must not read the 51.8 MiB v1 report, any archive, stream, manifest, result,
candidate, snapshot, or recipe. No unregistered v1 candidate may enter
initialization, proposal, ranking, or adoption.
...
- Code base before this experiment: `3408c883`.
- Authorizing v1 preregistration: `e3ca732b`; sealed implementation:
  `fe5e2c75`; registered result: `3408c883`.
- Exact source file:
...
- Compact-file and semantic `SmbInput` SHA-256:
  ...
  exactly 3,309 actions.
- Registered replay endpoint: alive `ExitKind::Ok`; maximum and endpoint
  `SmbProgressWatermark { world: 7, level: 0, progress: 270 }`; exactly 155,855
  frames; mechanical state `(world=7, level=0, progress=270,
  player_y_bucket=8, player_engine_state=8, dead=false, flag_active=false)`;
  frozen key `(7,0,270,8,8,state_fingerprint=55)`.
...
- Registered final action: opaque `ButtonChord { buttons: 16,
  hold_frames: 108 }`. Registered source probe: mask `0x00`, 45 frames,
  alive and survived.
- ROM SHA-256:
  `0b3d9e1f01ed1668205bab34d6c82b0e281456e137352e4f36a9b2cfa3b66dea`.
- Seed label `sol-restart-p270-normal-endpoint-frontier-harvest-v2`;
  label SHA-256
  `4e9ac2b9f6b21240453025245299f1470c5edfcc5545c0ce9a067117ec2e3524`;
  little-endian first-eight-byte master seed `4616949340756417102`.

The standalone binary is `smb-p270-endpoint-frontier-harvest`; positional
...
Use the sealed trace framing from v1 and record the newly computed trace hash.
...
source_index = first_8_bytes_as_little_endian_u64(action_digest) mod 3309
...
Each lane starts a new archive containing only the validated p270 source as
...
- one source replay: 155,855 frames;
- one source evidence probe: 45 frames; and
- thirteen target setups at 361 frames each: 4,693 frames.

Total hard bound: 1,727,313 frames. Record and reconcile every component with
...
`(7,0,270)`. Embed its exact `SmbInput`, lane, id, lineage, endpoint evidence,
