...
//! CLI entry point for the model-backed instrumentor subprocess.

use std::{ffi::OsString, io, path::PathBuf};

use instrumentor_agent::{AgentConfig, AgentError, run_agent};
...
fn parse_args(args: impl IntoIterator<Item = OsString>) -> Result<AgentConfig, AgentError> {
    ...
    let mut schema =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("schemas/instrumentor-decision.schema.json");
    let mut timeout_program = OsString::from("gtimeout");
    let mut codex_program = OsString::from("codex");
    let mut timeout_seconds = 1200_u64;
