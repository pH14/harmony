...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let baseline_nanos = baseline_elapsed.as_nanos();
    let projected_nanos = baseline_nanos
        .saturating_mul(u128::from(projected_replays))
        .checked_div(workers.get() as u128)
        .unwrap_or(u128::MAX);
    ...
    eprintln!(
        "projection before reduction: {projected_replays} candidate replays, {:.1} worker-hours at {} workers (conservative full-replay equivalents)",
        projected_nanos as f64 / 3_600_000_000_000.0,
        workers.get()
    );
    io::stderr().flush()?;

    let reduced = reduce_verified_sequence(
        &replay,
        genesis.clone(),
    ...
    let segments = reduced
        ...
        .iter()
        .zip(pairs)
        .map(|(segment, (entry_pair, exit_pair))| SegmentReport {
            index: segment.index,
            entry_pair,
            exit_pair,
