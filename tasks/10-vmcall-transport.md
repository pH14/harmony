# Task 10 — `consonance/vmcall-transport`: guest-side VMCALL transport shim

Read `tasks/00-CONVENTIONS.md` first. Touch only `consonance/vmcall-transport/`.
...
1. `cargo build -p vmcall-transport --all-features` and with default features — both clean on
...
2. **no_std proof**: the library builds for a bare-metal target where `std` is unavailable —
   ...
   `cargo build -p vmcall-transport --target x86_64-unknown-none` succeeds (this also compiles
   the real `vmcall` `asm!`). A library that accidentally pulls `std` fails here.
3. `cargo test -p vmcall-transport --all-features` — includes the loopback round-trip
...
4. A `proptest` (≥ 256 cases) that round-trips arbitrary service payloads through
   ...
   services produced.
5. `cargo clippy -p vmcall-transport --all-features --all-targets -- -D warnings` and
   `cargo fmt -p vmcall-transport -- --check` clean.
...
A branch `task/vmcall-transport` containing only `consonance/vmcall-transport/`, all gates green,
