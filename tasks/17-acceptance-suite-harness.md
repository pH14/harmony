# Task 17 — `consonance/acceptance-suite`: oracle runner & corpus manifest

Read `tasks/00-CONVENTIONS.md` first, then `docs/DETERMINISM-CORPUS.md` (the design this
implements). Touch `consonance/acceptance-suite/` **plus** the small additive `unison` accessor that O3
...
edit outside `consonance/acceptance-suite/` — O3 is unimplementable without it (current `Machine` exposes
...
std crate: a library plus a `acceptance-suite` binary.
...
The binary: `acceptance-suite run --manifest <path> [--item NAME] [--seed S]` runs the applicable
...
detector). A `acceptance-suite validate --manifest <path>` subcommand round-trips the manifest and
