# Task 86 — the real-game workload: Super Mario World under the fault-free exploration gate

> **FRONTIER · the held-out generalization test for the search seam (the Metroid demo made
> real).** Task 84 proves signal-beats-random on a maze whose depth, size, and branching are
> manifest knobs — a workload *designed* to be beatable, and the fixture task 70's selectors are
> tuned against. This task re-runs the same fault-free gate on a workload nobody can tune: a
> commercial SNES game (Super Mario World) running under a libretro core as an ordinary
> supervised Linux guest process. If task 70's selector only beats baseline on the maze it was
> tuned on, we measured overfitting, not search — this task is where we find out. It also lands
> the **billboard** (a per-frame, always-on PPU-state export in guest memory) that task 87
> (`film`) consumes to make replays visible.
>
...
`retro_run` + synthetic WRAM) — no ROM and no emulator anywhere in portable tests. The
...
**ROM provisioning (hard requirement).** The SMW ROM is copyrighted and is **never committed,
vendored, or fetched by any script in this repo**. The image build reads it from
`HARMONY_SMW_ROM=<path>` (user-supplied dump); when unset, the image builds without the game
...
**Core provisioning.** The libretro core is fetched at image-build time from a **commit-pinned
upstream** (like the kernel), never vendored. Core choice is the implementer's, documented in
`IMPLEMENTATION.md` with the trade-off: bsnes (GPL-3, accuracy, slow) vs snes9x
(non-commercial license, ~10× faster — throughput matters at box branch rates). A build-time
patch file against the pinned core (≤ ~100 lines, e.g. exporting a PPU-state accessor if the
core's `retro_get_memory_data` surface lacks VRAM) is permitted; copying core source into the
repo is not. Same no-copy discipline as bedrock.
...
- `dissonance/benchmark` — extend (do **not** fork) with the SMW report configuration; the
...
state currency, and a "checkpoint at Yoshi's Island 2 with a cape" is just an admitted cell.
...
1. **Draws inputs.** One byte of decision entropy via `Sdk::entropy_fill` per **input window**
   ...
   chord alphabet** (manifest): e.g. `RIGHT`, `RIGHT+Y` (run), `RIGHT+Y+B` (run-jump),
   `RIGHT+B` held (full jump), `A` (spin-jump), `LEFT`, `DOWN`, `UP`, neutral. Weights bias
   rightward (SMW levels do). `START`/`SELECT` are excluded (pausing burns budget). Per-frame
...
2. **Emits state registers** (once per window, via `state_set` — the R-L2 thin-SDK shape; the
   host owns cell interpretation): `REG_GAME_MODE`, `REG_LEVEL` (translevel), `REG_SCREEN`,
   `REG_X_BUCKET` (player X, bucketed ~256 px), `REG_POWERUP`, `REG_EXITS` (the game's
   activated-exit count — the depth metric), and `REG_FRAME` every vblank (the frame clock
   task 87 addresses film frames by). Addresses come from the SMWCentral RAM map (game mode
   `$7E0100`, translevel `$7E13BF`, player X `$7E0094-95`, powerup `$7E0019`, etc. — **the
   implementer verifies every address against the RAM map and unit-tests the decode against
   synthetic WRAM fixtures**; the exits-count address especially).
3. **Publishes the billboard** (task 87's enabling seam — built here because it must be part
   ...
   frame-coherent PPU snapshot into one pinned guest buffer: VRAM (64 KiB) + CGRAM (512 B) +
   OAM (544 B) + the WRAM window holding SMW's own PPU register mirrors (SMW writes scroll/
   mode/layer registers from RAM during NMI, which spares us a core patch for write-only PPU
   registers). The buffer opens with a self-describing header: magic, layout version, region
   offsets/lengths, and the current frame counter (so a reader can check read-vs-clock
   alignment). Its guest-physical address is published once at init via state registers —
   either a hugetlb mapping (one contiguous gpa) or a page-0 gpa table for a scattered
   buffer; implementer's choice, documented. **The billboard is unconditional** — always on,
   filmed or not — because the one-reproducer rule forbids a "render mode" that would fork
   the timeline; a ~65 KiB/frame memcpy is noise at emulation speeds and is simply part of
   the workload.
4. **Marks legibility events**: `assert_reachable` on first exit activated and on one named
   deep goal (suggested: clearing `#1 Iggy's Castle`). Markers, not bugs — zero fault
   vocabulary, exactly as task 84 rules.

## The cell key (host-side, retunable)

`LinkSensor` turns the registers into `(Moment, Feature)` on `LINK_STATE_CHANNEL`; the
campaign's `CellFn` keys cells on **(game mode, translevel, screen, x-bucket)** — the analog of
Antithesis's discretized `(x, y)` tuple. Depth = `REG_EXITS`. Because interpretation is
...
R-L2 logs), run one **diagnostic** configuration preferring `REG_POWERUP`/Yoshi at equal cells
...
(distinct cells and max exits) goes in `IMPLEMENTATION.md`, so a win cannot be claimed against
a saturated or broken control.

## Acceptance gates

1. **Portable (macOS + Linux):** play-agent logic against the mock core — chord decode from a
   fixed entropy stream reproduces a fixed input tape; RAM-map decode against synthetic WRAM
...
3. **Box gate — the measurement:** a committed `dissonance/benchmark/SMW-EXPLORATION-REPORT.md`
...
4. **The verdict:** **PASS** = signal strictly beats pure-random on **both** distinct cells and
   exits (greater medians, non-overlapping IQRs), against a demonstrably live control. A
