...
// order. An oracle can then read back one contiguous span of recent keys with
// a single range request instead of the whole prefix.
...
var entryPattern = regexp.MustCompile(
	`^museum/([0-9]+)/key-([0-9]+)\tvalue-([0-9]+)-([0-9]+)$`,
)
...
func resumeSequences(path string) (map[int]int, error) {
	...
	for scanner.Scan() {
		worker, sequence, ok := parseEntry(scanner.Text())
		if !ok {
...
func parseEntry(line string) (int, int, bool) {
