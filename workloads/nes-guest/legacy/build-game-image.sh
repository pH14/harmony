...
# FLOW_AGENT_BIN pattern); by default build it from workloads/nes-guest/.
if [ -n "${PLAY_AGENT_BIN:-}" ]; then
    echo "== game image: using prebuilt play-agent: $PLAY_AGENT_BIN"
    AGENT_BIN=$PLAY_AGENT_BIN
else
    echo "== game image: building play-agent (workloads/nes-guest)"
    AGENT_BIN=$(bash "$GUEST_DIR/../../workloads/nes-guest/build.sh" | tail -1)
fi
...
install -m 0755 "$package_dir/game-init.sh" "$GAMEROOT/init"
