...
: "${HARMONY_NOVA_ROM:?set HARMONY_NOVA_ROM to the pinned built nova.nes (from workloads/nes/scripts/build-nova-rom.sh)}"
[ -f "$HARMONY_NOVA_ROM" ] || {
...
if [ -n "${PLAY_AGENT_BIN:-}" ]; then
    ...
    rm -rf "$agent_target"
    (
        cd "$GUEST_DIR/../../workloads/nes-guest"
        RUSTC_BOOTSTRAP=1 \
...
install -m 0755 "$package_dir/nova-game-init.sh" "$nova_root/init"
