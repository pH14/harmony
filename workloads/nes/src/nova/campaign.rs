...
impl NovaMachineKind for QuickNesMachine {
    ...
    fn new_nova_target(game: &NovaGame<Self>) -> Result<NovaTarget<Self>, String> {
        NovaTarget::from_rom_bytes_headless_at_level(
            &game.rom,
            &game.runtime.core_path,
            &game.runtime.core_sha256,
            game.level,
        )
...
impl crate::film::FilmSource for NovaGame {
    fn render_film(
        &self,
        input: &NovaInput,
        tail_frames: u32,
    ...
    fn film_action_frames(&self, input: &NovaInput) -> u64 {
        input
            .actions
            .iter()
...
mod tests {
    ...
    #[test]
    fn legacy_consonance_v1_identity_is_rejected() {
        let game = NovaGame::new(&[1, 2, 3], Path::new("core.so"), &"a".repeat(64));
        ...
        policies.insert(EMULATOR_BACKEND_FIELD.to_owned(), legacy_identity);
        let error = game
            .resolve_recorded(&policies)
            .expect_err("the pre-extraction VM identity must not be reinterpreted");
        assert!(error.to_string().contains(EMULATOR_BACKEND_FIELD));
    }
