...
use nes_workload::{
    ...
    thwaite::{
        ...
        campaign::{
            ThwaiteCampaignConfig, ThwaiteCampaignOrigin, ThwaiteGame,
        ...
        I: IntoIterator<Item = OsString>,
    {
        ...
        if action_limit == 0 || action_limit > nes_workload::thwaite::archive::MAX_THWAITE_ACTIONS {
            return Err(format!(
                ...
                nes_workload::thwaite::archive::MAX_THWAITE_ACTIONS
            )
            .into());
        }
        Ok(Self {
...
fn campaign_config(args: &Args) -> ThwaiteCampaignConfig {
    ThwaiteCampaignConfig {
...
fn run_qualified_campaign(
    ...
    let (live, checkpoint) = run_thwaite_campaign_checkpointed(
        game,
        config,
        &ThwaiteCampaignOrigin::Genesis,
        &mut stream,
        Some(&mut progress),
    )?;
    stream.flush()?;
    ...
    let (replayed, replayed_checkpoint) =
        replay_thwaite_campaign_checkpointed(game, &stream_bytes, None, None)?;
    ...
    if !replay_verified {
        return Err("Thwaite campaign replay diverged".into());
    }
    ...
    let media = render_video(game, &render_input, output, RENDER_TAIL_FRAMES)?;
    if media.video.input_endpoint != rendered_endpoint.state {
        return Err("video-enabled replay changed Thwaite rendered endpoint".into());
    }
    ...
    let summary = json!({
        ...
        "town_lost": champion_endpoint.state.game_over(),
        "survived_all_days": champion_endpoint.state.survived_all_days(),
        ...
        "champion_endpoint": champion_endpoint.state,
        "champion_evidence": champion_endpoint.evidence,
        "champion_frames": champion_endpoint.frame_count,
        ...
        "rendered_input_frames": render_input
            ...
            .sum::<u64>(),
        "render_tail_frames": RENDER_TAIL_FRAMES,
        "render_excerpt": render_input != champion,
        "rendered_endpoint": rendered_endpoint.state,
...
fn write_headless_observation(
    ...
    output: &Path,
    stem: &str,
) -> Result<WitnessEndpoint, Box<dyn Error>> {
    let mut target = game
        .new_target()
    ...
    for action in &input.actions {
        ...
        if target.exit_kind() != ExitKind::Ok {
            return Err("retained Thwaite witness crashed during replay".into());
        }
    }
    fs::write(
