...
//! Render a recorded Mega Man 2 input as an H.264 MP4 with game audio.
//!
//! The replay drives the same [`Mm2Target`] the searcher drives, so the film
...
//!
//! The chain prefix is the physical input that reaches the stage select, so a
//! searched castle input plays back from power-on.
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Capture begins at stage genesis, so the film opens on the first frame of
    // the searched input rather than on the walk that reached the stage.
