...
use nes_workload::thwaite::target::{ThwaiteInput, ThwaiteTarget, decode_state, setup_tape};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rom = PathBuf::from(
        args.next()
            .or_else(|| env::var_os("HARMONY_THWAITE_ROM"))
            .ok_or("usage: thwaite-probe [ROM] [buttons:hold,...]")?,
    );
    let schedule = args
        .next()
    ...
    if env::var_os("HARMONY_THWAITE_TRACE_SETUP").is_some() {
        ...
        for (index, action) in tape.iter().enumerate() {
            ...
            let state = decode_state(&machine.read_wram()?)?;
    ...
    let mut target = ThwaiteTarget::from_rom_bytes_headless(&rom_bytes, &core, &core_sha256)?;
    println!(
        "genesis {}",
        serde_json::to_string(&target.mechanical_state())?
    );

    if env::var_os("HARMONY_THWAITE_CORRECTNESS").is_some() {
    ...
    let input = ThwaiteInput { actions };
    for (index, action) in input.actions.iter().enumerate() {
        ...
        println!(
            ...
            index,
            serde_json::to_string(action)?,
            serde_json::to_string(&target.last_action_observations().to_vec())?
        );
        if target.is_game_over() || target.exit_kind() != nes_workload::target::ExitKind::Ok {
            break;
        }
    }
    println!(
...
fn run_correctness_probes(
    ...
    let genesis = target
        .snapshot()
        .ok_or("could not snapshot the Thwaite gameplay genesis")?;
    let controls = [
        ("noop", 0x00),
        ("a", 0x01),
        ("b", 0x02),
    ...
    target.restore(&genesis)?;
    target.apply(&ButtonChord::new(0x80, 6));
    let branch = target
        .snapshot()
        .ok_or("could not snapshot the aiming branch")?;
    let continuation = ButtonChord::new(0x01, 12);
    ...
    if first != second {
        return Err("Thwaite same-continuation snapshot replay diverged".into());
    }
    target.restore(&branch)?;
    ...
    if first != third {
        return Err("Thwaite restore after a different branch diverged".into());
    }
    println!(
        "snapshot_restore {}",
        serde_json::to_string(&serde_json::json!({
    ...
    if before_probe != after_probe || target.exit_kind() != nes_workload::target::ExitKind::Ok {
        return Err(
            "Thwaite admission probe failed its live-RAM and cached-state restoration checks"
                .into(),
        );
    }
    ...
    if (target.observe(), target.fingerprint()) != after_probed_continuation {
        return Err("Thwaite post-probe continuation diverged".into());
    }
    target.restore(&genesis)?;
    println!(
        "admission_probe {}",
    ...
    let clean = ThwaiteTarget::from_rom_bytes_headless(rom, core, core_sha256)?;
    if clean.mechanical_state() != genesis.state() {
        return Err("Thwaite clean reset disagreed with sealed genesis".into());
    }
