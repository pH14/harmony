...
impl crate::film::Filmable for StbGame {
    fn film(
        &self,
        input: &StbInput,
        tail_frames: u32,
...
impl crate::film::Endpointed for StbGame {
    fn headless_endpoint(&self, input: &StbInput) -> Result<serde_json::Value, Box<dyn Error>> {
        let mut target = self
            .new_target()
            .map_err(|error| -> Box<dyn Error> { error.into() })?;
        target.reset();
        for action in &input.actions {
            target.apply(action);
            if target.exit_kind() != ExitKind::Ok {
                return Err("the recorded STB input crashed during headless replay".into());
            }
        }
