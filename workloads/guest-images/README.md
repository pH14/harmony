...
The `Docker` and `K3s` jobs in `Checks / Harmony Workloads / OCI` run each
nested recipe twice through the canonical x86 runtime and compare complete
serial logs, application and runtime exit statuses, and readiness evidence. The
Docker recipe launches the official PostgreSQL image with Docker's bundled
`runc`; the K3s recipe also checks pod-to-PostgreSQL traffic through the guest
CNI. Both require an exact qualified platform artifact and preserve image hashes
and run records. They run on a schedule and on dispatch, because building a
pinned workload image and booting it twice exceeds the pull request budget. Run
them independently on a proposed branch with:

```sh
gh workflow run harmony-workloads-oci-checks.yml --ref YOUR_BRANCH
```
