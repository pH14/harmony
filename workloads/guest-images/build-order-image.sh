...
# output swapped, so all three benchmark images share one determinism closure.
# Produces consonance/harmony-linux/build/initramfs-order.cpio.gz.
#
# This is `build-postgres-image.sh` with two additions, kept in lockstep with it:
#   1. a static `order-super` compiled from order-super.c and installed at
#      /order-super (the supervised process carrying the planted bug), and
...
if [ "$(id -u)" != "0" ]; then
    echo "FAIL: build-order-image.sh must run as root (mke2fs -d bakes uid-70 ownership)." >&2
    exit 1
fi
...
FIXED_UUID="deadbeef-0000-0000-0000-000000000062"   # pinned ext4 UUID (bug 2, distinct)
EXT4_SIZE=96M                                   # cluster (~22M) + workload WAL headroom
WORKLOAD_N=20                                   # fixed insert/select iterations

PGROOT=$BUILD_ROOT/order-root                # the assembled guest rootfs
PG_STAGE=$BUILD_ROOT/order-stage             # extracted .debs
STAGEFS=$BUILD_ROOT/order-stagefs            # initdb output, baked into the ext4
...
echo "== order image: extracting pinned PostgreSQL $PG_VERSION .debs"
rm -rf "$PG_STAGE"
...
echo "== order image: building static busybox ($BUSYBOX_VERSION)"
extract_busybox
mkdir -p "$BBOBJ"
...
# --- 1b. the planted-bug supervisor: static order-super -------------------
...
echo "== order image: compiling static order-super"
cc -static -O2 -Wall -Wextra -fno-asynchronous-unwind-tables \
    -o "$BUILD_ROOT/order-super" "$workload_dir/order-super.c"
[ -x "$BUILD_ROOT/order-super" ] || { echo "FAIL: order-super did not build" >&2; exit 1; }

# --- 2. assemble the guest rootfs (mirrors build-postgres-image.sh) ----------
echo "== order image: assembling rootfs"
rm -rf "$PGROOT"
...
cp "$BUILD_ROOT/order-super" "$PGROOT/order-super"
chmod 0755 "$PGROOT/order-super"
...
{ ldd "$PGBIN/postgres"; ldd "$PGBIN/psql"; ldd "$PGBIN/pg_ctl"; } 2>/dev/null \
    | awk '/=> \// {print $3}' | sort -u >"$BUILD_ROOT/order-libs.txt"
echo /lib/x86_64-linux-gnu/libnss_files.so.2 >>"$BUILD_ROOT/order-libs.txt"
while read -r so; do
    [ -e "$so" ] && cp -L "$so" "$PGROOT/lib/x86_64-linux-gnu/$(basename "$so")"
done <"$BUILD_ROOT/order-libs.txt"
...
echo "== order image: initdb (build-time, once) + determinism overlay"
rm -rf "$STAGEFS"
...
setpriv --reuid="$BUILD_UID" --regid="$BUILD_UID" --clear-groups env LC_ALL=C.UTF-8 TZ=UTC \
    ...
    -A trust -U postgres -N >"$BUILD_ROOT/order-initdb.log" 2>&1 \
...
echo "== order image: baking fixed-UUID ext4 with the cluster"
...
install -m 0755 "$workload_dir/order-init.sh" "$PGROOT/init"

# --- 5. pack the initramfs (sorted, fixed mtime, owner 0:0, gzip -n) ----------
echo "== order image: packing initramfs"
find "$PGROOT" -mindepth 1 -exec touch -hcd @0 {} +
( cd "$PGROOT" && find . -mindepth 1 -print0 | LC_ALL=C sort -z \
    | cpio --null -o -H newc --owner=0:0 --quiet ) | gzip -n -9 >"$ART_DIR/initramfs-order.cpio.gz"
echo "ok: $ART_DIR/initramfs-order.cpio.gz ($(du -h "$ART_DIR/initramfs-order.cpio.gz" | cut -f1))"
