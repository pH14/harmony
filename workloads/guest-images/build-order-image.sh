...
# Build the **bug-2 (ordering/interrupt-timing) benchmark OCI layout**: the
# bare-Postgres image (a static busybox + a real PostgreSQL 17 install + a
# pre-`initdb`'d cluster) plus the planted-bug supervisor `order-super`. A
# clone of the campaign recipe with the supervisor and output swapped, so all
# three benchmark images share one determinism closure. The platform package
# owns the kernel, mounts, PID 1, and final guest termination.
#
...
if [ "$(id -u)" != "0" ]; then
    echo "FAIL: build-order-image.sh must run as root (the OCI image preserves uid-70 ownership)." >&2
    exit 1
fi
...
STAGEFS=$BUILD_ROOT/order-stagefs            # initdb output, copied into PGDATA
...
echo "== order image: copying the deterministic cluster into PGDATA"
...
OCI_OUT=$ART_DIR/oci-images/order.oci
rm -rf "$OCI_OUT"
...
python3 "$workload_dir/oci-package.py" \
    ...
    --env HARMONY_POSTGRES_VARIANT=order \
