...
The race is independent of block cloning. It reproduces on 2.1.x, where block cloning does
not exist; cloning widens the window, and `zfs_bclone_enabled=0` leaves the race in place.
The `dnode_is_dirty()` pattern became practically hittable when
`zfs_dmu_offset_next_sync=1` became the default in **2.1.4**.
...
- **Fix**: commit
  ...
  ("dnode_is_dirty: check dnode and its data for dirtiness", PR
...
- Upstream hits it with brute parallelism in seconds-to-minutes; the benchmark question is
  ...
  dial), `zfs_txg_timeout`, file count/size, number of concurrent loops.
