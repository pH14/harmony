...
//! finding needs the meaning of each declared assertion, its reporting hook, and
...
impl Declarations {
        ...
        for (number, line) in text.lines().enumerate() {
            ...
            match keyword {
                ...
                "describe" => {
                    ...
                    match what.as_str() {
                        "node" => {
                            if node_descriptions.insert(subject.clone(), text).is_some() {
                                return Err(format!(
                                    "bundle line {at}: node {subject:?} is described twice"
                                ));
                            }
                        }
                        "hook" => {
                            ...
                            if hook_descriptions.insert(id, text).is_some() {
                                return Err(format!(
                                    "bundle line {at}: hook {id} is described twice"
                                ));
                            }
                        }
                        other => {
    ...
    fn check_unique(&self) -> Result<(), String> {
        let mut node_names: Vec<_> = self.nodes.iter().map(|node| node.name.as_str()).collect();
        node_names.sort_unstable();
        if node_names.windows(2).any(|pair| pair[0] == pair[1]) {
            return Err("a fault bundle declares one node name twice".to_owned());
        }
        for assertion in &self.assertions {
            if let Some(id) = assertion.reported_by_hook
                && !self.hooks.iter().any(|hook| hook.id == id)
            {
                return Err(format!(
                    "assertion {} names undeclared reporting hook {id}",
                    assertion.id
                ));
            }
        }
...
describe node service the supervised test service
ready /opt/harmony/ready.sh
hook 3 /opt/harmony/hooks.sh 3
describe hook 3 inspect the test service counter
assert always 2 from 3 the test service counter is nonnegative
assert sometimes 24 the checker reached a verdict
diagnostic counter cat /run/counter.out
";
    ...
    fn a_bundle_declares_nodes_hooks_properties_and_diagnostics() {
        ...
        assert_eq!(
            ...
            Some("the supervised test service")
        );
        ...
        assert_eq!(always.meaning, "the test service counter is nonnegative");
        assert_eq!(
            declarations.assertion(24).expect("assertion 24").kind,
            AssertionKind::Sometimes
        );
        assert_eq!(
            declarations.diagnostic("counter").expect("diagnostic").argv,
            ["cat", "/run/counter.out"]
        );
    }
    ...
    fn malformed_declarations_name_their_line() {
        for (text, fragment) in [
            ...
            ("node a /bin/a\nnode a /bin/b\n", "node name twice"),
            (
                "node a /bin/a\ndescribe node a first\ndescribe node a second\n",
                "described twice",
            ),
            (
                "hook 1 /bin/a\ndescribe hook 1 first\ndescribe hook 1 second\n",
                "described twice",
            ),
            (
                "assert always 1 from 2 a claim\n",
                "undeclared reporting hook",
            ),
            ("hook 1 /bin/h\nhook 1 /bin/g\n", "hook id twice"),
            (
