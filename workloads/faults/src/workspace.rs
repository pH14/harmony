...
pub fn publish(
    ...
    for bug in &reports {
        ...
        if bug.observations.stop == crate::target::FaultStop::Unexpected {
            return Err(format!(
                "bug report {} has an unexpected stop; refusing workspace publication",
                bug.bug
            )
            .into());
        }
    ...
    for (index, bug) in reports.iter().enumerate() {
        ...
        records.push(Record::Moment(Box::new(MomentRecord {
            ...
            stop: Some(match bug.observations.stop {
                crate::target::FaultStop::Deadline => StopReason::VirtualDeadline,
                ...
                crate::target::FaultStop::Unexpected => {
                    return Err(format!(
                        "bug report {} has an unexpected stop; refusing workspace publication",
                        bug.bug
                    )
...
mod publish_tests {
    ...
    #[test]
    fn an_unexpected_stop_is_rejected_before_initial_publication() {
        let directory = tempfile::tempdir().expect("temp dir");
        let observations = FaultObservations {
            moment: 50,
        ...
        .expect_err("unexpected stop cannot become a durable endpoint");
        assert!(error.to_string().contains("unexpected"), "{error}");
        assert!(
            !directory.path().join(JOURNAL_DIR).exists(),
            "unexpected stop must not publish facts or endpoint records"
        );
    }
