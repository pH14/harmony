...
| `hidden_state` | Needs `gauntlet`. A hidden value from 0 to 63 turns every action by an amount fixed per room and value. An action clock advances by one per action modulo 64, and each room entry redraws the hidden value from the room, the clock and the old value. A recorded suffix of one to three actions replayed from another holder lands about three times in ten, and a retry from the same holder about one time in thirteen. | Enemy and object state that the key does not see and that differs between two holders in one cell. |
| `rise_slows_farms` | Needs `gauntlet`. Once every item is held, farms give health on one arrival in 80 instead of one in eight. | Refills that are far out of the way after the last item. |
| `costly_walk` | Needs `gauntlet`. The first item is the outer room farthest from the door. A holder with the first item but not every item loses one health on each entry into an outer room in the half nearest the door, the door room included, other than the start and item rooms. An entry with no health left kills the run. Health stops at the entry-to-goal room count plus 2 plus the room count from the last item to the nearest farm. | The walk back from a far item costs energy, so full energy exists only far from the entry. |
...
| Map | `width` and `height` 2–8; any `layout`; `loops` 0–16; `corridor` and `shaft` 1–4; `inner` from 2 to two fewer than the room count; `items` 1–9, and above 1 only with two outer rooms to spare and without farms unless `gauntlet`; `farms` 0–8 with `farm_cap` 1–63, and `farm_cap` only with farms; `boss_stock` 0–`farm_cap`, 1–6 with `boss_hits_back`, and not in chain stages; `timing` 0 or 2–16; `item_optional` needs one item and no boss; `locked` needs one item, no optional item, no boss, and an outer room for the key besides the start and the door room; `gauntlet` needs `items` 2 or more and farms; `hidden_state`, `rise_slows_farms` and `costly_walk` need `gauntlet`. |
