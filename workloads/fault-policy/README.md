...
workload-defined hook run (`RunHook`), a hold at an execution place
(`ProcPark`), and execution callbacks (`ProcEventKill` and `ProcEventPark`).
...
the one target encoding a host package and an in-guest agent both read.
