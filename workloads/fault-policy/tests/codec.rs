...
fn truncations_of_a_valid_blob_never_panic() {
    let mut policy = fault_policy::FaultPolicy::none();
    policy
        .set_class(
            fault_policy::DecisionClass::BlockIo,
            1,
            3,
            &[
                fault_policy::Fault::BlockEio,
                fault_policy::Fault::BlockTorn(8),
            ],
        )
        .unwrap();
    let spec = EnvSpec::Recorded {
        ...
        overrides: BTreeMap::from([
            (
                1,
                Action::Guest(Answer::Fault(fault_policy::Fault::BlockEio)),
            ),
            (
                4,
        ...
        standing: vec![fault_policy::StandingFault {
            class: fault_policy::DecisionClass::NetFlow,
            target: vec![0, 1, 2, 3],
            window: (10, 20),
        }],
...
fn host_fault_decode_rejects_zero_clock_rate_denominator() {
    let f = HostFault::SetClockRate(fault_policy::Ratio::new(3, 2).unwrap());
