...
fn seeded_and_seeded_recorded_baseline_agree() {
    ...
    policy
        .set_class(
            fault_policy::DecisionClass::NetFlow,
            1,
            2,
            &[
                fault_policy::Fault::NetReset,
                fault_policy::Fault::NetLatency(Span(7)),
            ],
        )
        .unwrap();
    ...
    let mut recorded = fault_policy::EnvSpec::Seeded { seed, policy }.materialize();
    for p in &seq {
    ...
    let spec = EnvSpec::Recorded {
        ...
        overrides: BTreeMap::from([(
            ...
            Action::Host(HostFault::CorruptMemory {
                gpa: 0x1000,
                mask: fault_policy::BitMask(0xFF),
            }),
        )]),
        standing: vec![],
