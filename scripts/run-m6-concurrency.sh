...
env CARGO_TARGET_DIR="$run_dir/rust-target" cargo build \
    --manifest-path "$repo_root/consonance/harmony-linux/concurrency-suite/rust-lost-update/Cargo.toml" \
    --release
(
    cd "$repo_root/consonance/harmony-linux/concurrency-suite/go-publish-before-init"
...
"$repo_root/scripts/m6-independent-oracle.py" \
    "$report" \
    "$repo_root/consonance/harmony-linux/concurrency-suite/m6-plan.json" \
