...
the resulting library as `/usr/lib/libvoidstar.so`; `HARMONY_DEVICE_PATH` exists only to
support non-privileged integration tests and defaults to the ABI path `/dev/harmony`.
