...
# harmony-linux/build/initramfs-campaign.cpio.gz.
#
...
echo "== campaign image: extracting pinned PostgreSQL $PG_VERSION .debs"
rm -rf "$PG_STAGE"
...
echo "== campaign image: building static busybox ($BUSYBOX_VERSION)"
extract_busybox
mkdir -p "$BBOBJ"
...
# --- 1b. the planted-bug supervisor: static campaign-super -------------------
...
echo "== campaign image: compiling static campaign-super"
cc -static -O2 -Wall -Wextra -fno-asynchronous-unwind-tables \
    -o "$BUILD_ROOT/campaign-super" campaign-super.c
[ -x "$BUILD_ROOT/campaign-super" ] || { echo "FAIL: campaign-super did not build" >&2; exit 1; }
...
cp "$BUILD_ROOT/campaign-super" "$PGROOT/campaign-super"
chmod 0755 "$PGROOT/campaign-super"
...
{ ldd "$PGBIN/postgres"; ldd "$PGBIN/psql"; ldd "$PGBIN/pg_ctl"; } 2>/dev/null \
    | awk '/=> \// {print $3}' | sort -u >"$BUILD_ROOT/campaign-libs.txt"
echo /lib/x86_64-linux-gnu/libnss_files.so.2 >>"$BUILD_ROOT/campaign-libs.txt"
while read -r so; do
    [ -e "$so" ] && cp -L "$so" "$PGROOT/lib/x86_64-linux-gnu/$(basename "$so")"
done <"$BUILD_ROOT/campaign-libs.txt"
...
echo "== campaign image: initdb (build-time, once) + determinism overlay"
rm -rf "$STAGEFS"
...
setpriv --reuid="$BUILD_UID" --regid="$BUILD_UID" --clear-groups env LC_ALL=C.UTF-8 TZ=UTC \
    ...
    -A trust -U postgres -N >"$BUILD_ROOT/campaign-initdb.log" 2>&1 \
...
install -m 0755 "$LINUX_DIR/campaign-init.sh" "$PGROOT/init"
