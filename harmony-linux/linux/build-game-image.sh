...
# /init. Produces harmony-linux/build/initramfs-game.cpio.gz.
#
...
# FLOW_AGENT_BIN pattern); by default build it from harmony-linux/play-agent/.
if [ -n "${PLAY_AGENT_BIN:-}" ]; then
    echo "== game image: using prebuilt play-agent: $PLAY_AGENT_BIN"
    AGENT_BIN=$PLAY_AGENT_BIN
else
    echo "== game image: building play-agent (harmony-linux/play-agent)"
