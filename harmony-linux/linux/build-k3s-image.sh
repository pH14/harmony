...
#
# **What runs (see harmony-linux/linux/IMPLEMENTATION.md + tasks/49-*).** One single-vCPU
...
verify_pin "$K3S_BIN" "$K3S_BIN_SHA256" "run 'make -C harmony-linux fetch' first"
if [ ! -f "$PG_IMAGE_TAR" ] || [ ! -s "$PG_IMAGE_TAR" ]; then
    echo "FAIL: $PG_IMAGE_TAR missing/empty — run 'make -C harmony-linux fetch' on the box" >&2
    echo "      (ctr+network; integrity anchored by $POSTGRES_IMAGE_INDEX_DIGEST)." >&2
    exit 1
fi
if [ ! -f "$PAUSE_IMAGE_TAR" ] || [ ! -s "$PAUSE_IMAGE_TAR" ]; then
    echo "FAIL: $PAUSE_IMAGE_TAR missing/empty — run 'make -C harmony-linux fetch' on the box" >&2
...
# `harmony-linux/flow-agent/build-static.sh`; bake it in when its path is passed via
...
# --- task 49 determinism overlay (see harmony-linux/linux/IMPLEMENTATION.md) ---
...
mkdir -p "$ART_DIR"   # the artifact dir (harmony-linux/build) may not exist on a fresh checkout
