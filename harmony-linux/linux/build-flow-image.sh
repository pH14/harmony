...
# harmony-linux/build/initramfs-flow.cpio.gz.
#
...
#   harmony-linux/linux/build-flow-image.sh
...
# pattern); by default build it from harmony-linux/flow-agent/.
if [ -n "${FLOW_AGENT_BIN:-}" ]; then
    echo "== flow image: using prebuilt flow-agent: $FLOW_AGENT_BIN"
    AGENT_BIN=$FLOW_AGENT_BIN
else
    echo "== flow image: building flow-agent (harmony-linux/flow-agent)"
