...
# container-class bzImage (the §capability audit in harmony-linux/linux/IMPLEMENTATION.md
...
if [ ! -f "$DOCKER_TGZ" ]; then
    echo "FAIL: $DOCKER_TGZ missing — run 'make -C harmony-linux fetch' first" >&2
    exit 1
fi
...
# --- task 38 determinism overlay (see harmony-linux/linux/IMPLEMENTATION.md) ---
