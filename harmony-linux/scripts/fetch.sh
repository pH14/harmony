...
fetch_postgres_image() {
    ...
    if ! command -v ctr >/dev/null 2>&1; then
        echo "skip: $out — needs 'ctr' (containerd). Run 'make -C harmony-linux fetch' on the" >&2
...
# single-image tar (harmony-linux/dl/k3s-pause-image.tar). Every pod needs the sandbox
