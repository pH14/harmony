...
//!
//! `harmony preflight` reports which support-matrix cell (docs/DETERMINISM.md
//! §4) the current host occupies and whether the guest artifacts are installed.
//! `harmony oci run` boots an OCI container image inside the deterministic
//! hypervisor. `harmony search` explores a workload and leaves a workspace.
//! The workspace verbs — `findings`, `branches`, `inspect`, `fork`, `run`,
//! `exec`, `export` — open that durable history one command at a time.
...
#[command(
    ...
    after_help = "A search writes a workspace; -w opens it.\n\
                  \n\
                  \x20 harmony search --package faults app.oci --out W --seed 7 --executions 100\n\
                  ...
                  \x20 harmony -w W exec trace --within 1s -- sh -c 'cat /run/amcheck.out'\n\
