...
//! bounded amount of guest time, run a guest command, and cite the evidence.
//!
...
fn exec_request(args: &ExecArgs) -> Result<ExecRequest, Box<dyn Error>> {
    let target = match (&args.branch, &args.at) {
        (Some(_), Some(_)) => {
            return Err("name a branch to run on it, or --at a point to probe it, not both".into());
        }
        (Some(branch), None) => Selector::BranchHead(branch.clone()),
        (None, Some(at)) => Selector::parse(at)?,
        (None, None) => {
    ...
    match selector {
        ...
        Selector::BranchAt(name, at) => {
            ...
            workspace
                ...
                .find(|record| record.virtual_time == *at)
                .map(|record| record.id.clone())
                .ok_or_else(|| {
                    format!(
...
fn guest(
    ...
    Ok(Box::new(
        faults_workload::investigate::live::ConsonanceGuest::boot(
            &artifacts.kernel,
            &artifacts.initramfs,
            &config,
        )?,
...
/// Give every virtual time a duration a reader can act on, keeping the
/// nanoseconds beside it. Verbs build their documents in nanoseconds; both
/// renderings then carry both forms.
    ...
    match value {
        serde_json::Value::Object(fields) => {
                ...
                .and_then(serde_json::Value::as_u64)
            {
                ...
                fields.insert("virtual_time".to_owned(), format_duration(nanos).into());
            }
